...
**テスト フェールオーバー**コマンドレットは、Windows Server バージョン 1709 で発表しました。 これらと Windows Server 2019 がも含まれています。 ここでは、一時的にバックアップの回復先ボリュームの読み取り/書き込みのスナップショットをマウントし、テストなどをサポートします。参照してください https://aka.ms/srfaqの詳細。

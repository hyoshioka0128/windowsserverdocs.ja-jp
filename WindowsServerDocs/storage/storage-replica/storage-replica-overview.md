...
Windows Server バージョン 1709 で、**テスト フェールオーバー**コマンドレットが追加されました。 ここでは、一時的にバックアップの回復先ボリュームの読み取り/書き込みのスナップショットをマウントし、テストなどをサポートします。参照してください https://aka.ms/srfaqの詳細。
...
-   記憶域レプリカは、バックアップではありません。 一部の IT 環境では、毎日のバックアップと比較して、データ損失ゼロのオプションがあるため、レプリケーション システムをバックアップ ソリューションとしてデプロイしています。 記憶域レプリカでは、ボリューム上の全データ ブロックに対するすべての変更が、変更の種類とは無関係にレプリケートされます。 ユーザーは、ボリュームからすべてのデータを削除した場合記憶域レプリカにレプリケートされます削除瞬時にその他のボリュームでは、両方のサーバーからのデータの完全な削除します。 記憶域レプリカを、特定の時点でのバックアップ ソリューションの代わりに使用しないでください。  

...
- **NFS バージョン4.1 のサポート**。 このプロトコルバージョンには、次の機能強化が含まれています。
  ...
  - は、RPCSEC \_ GSS プロトコルをサポートし、より強力なセキュリティを提供し、クライアントとサーバーがセキュリティをネゴシエートできるようにします。
...
- Windows Server 2012 を実行している1台以上のコンピューターで、nfs サーバーと nfs クライアントの2つの主要サービスをインストールします。 これらのコンポーネントは、同じコンピューターまたは別のコンピューターにインストールできます。
...
- RPCSEC \_ GSS
  ...
  - **Krb5p**: Kerberos Version 5 プロトコルを使用して、プライバシーを暗号化して NFS トラフィックを認証します。
- 認証 \_ システム

また、[サーバー認証 (AUTH SYS)] を使用しないように選択することもできます。これにより、マップされていない \_ ユーザーアクセスを有効にすることができます。 マップされていないユーザーアクセスを使用する場合は、割り当てられていないユーザーアクセスを UID/GID (既定値) または匿名アクセスを許可するように指定できます。
...
2. サーバー マネージャーが自動的に起動します。 自動的に起動しない場合は、[**スタート**] を選択し、「 **servermanager.exe**」と入力して、[**サーバーマネージャー**] を選択します。
...
次の Windows PowerShell コマンドレットでは、NFS ファイル共有を作成することもできます (ここ `nfs1` で、は共有の名前、 `C:\\shares\\nfsfolder` はファイルパスです)。

...
[既定のデータ重複除去ジョブ スケジュール](understand.md#job-info)は、(バックアップ[使用法の種類**に対して有効な**優先順位最適化](understand.md#usage-type-backup)ジョブを除いて) 推奨されるワークロードで適切に動作し、可能な限り悪影響を及ぼさないように設計されています。 ワークロードには、サイズの大きいリソース要件がある場合、ジョブがアイドル状態の時間内にのみ実行されるようにしたり、データ重複除去ジョブが使用できるシステム リソースの量を増減したりできます。
...
3. 高優先順位で、システム上のすべての CPU と使用可能なメモリを使用して午後 7時 00分に実行される、夜間の最適化ジョブを作成します。
    ...
    >[!NOTE]  
    > *に提供される* の`System.Datetime`日付`-Start`部分は (過去日付である限り) 使用されませんが、*時間*部分はジョブを開始する時間を指定します。
...
<table>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td><em>開始</em>するために提供された <code>System.Datetime</code> の&#39;<em>日付</em>部分は (過去の場合に限り) 無関係ですが、<em>時刻</em>部分ではジョブをいつ開始するかを指定します。</td>
        </tr>
        <tr>
...
<table>
    ...
    <tbody>
        <tr>
            ...
            <td>チャンクがチャンク ストアのホット スポット セクションに複製される前に参照される回数。 ホットスポットセクションの値は、アクセス時間を向上させるために、頻繁に参照される &quot;ホット&quot; チャンクに複数のアクセスパスがあることを示しています。</td>
            <td>正の整数</td>
...
<a id="faq-use-responsibly"></a>**データ重複除去設定を変更しましたが、ジョブの速度が遅いか、完了していないか、ワークロードのパフォーマンスが低下しています。なぜでしょうか。**  

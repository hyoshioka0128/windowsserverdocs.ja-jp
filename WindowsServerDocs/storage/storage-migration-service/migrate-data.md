...
2. **[資格情報の入力]** ページで、移行元のサーバーで動作する管理者の資格情報を入力し、 **[次へ]** を選択します。

   > [!NOTE]
   > Linux Samba サーバーからの移行を選択した場合は、SSH パスワードまたは秘密キーを提供する追加の手順があります。

3. **[デバイスの追加]** を選択し、移行元サーバーの名前を入力して、[ **OK]** を選択します。 <br>インベントリを作成する他のすべてのサーバーについて、この手順を繰り返します。

   > [!NOTE]
   > フェールオーバークラスターから移行することを選択した場合は、クラスター化されたファイルサーバーリソースの名前を指定します。
...
2. **[変換先のデバイスとマッピングの追加]** ページに、最初の移行元サーバーが表示されます。 移行先のサーバーの名前を入力し、 **[デバイスのスキャン]** を選択します。
   ...
   > [!NOTE]
   > フェールオーバークラスターへの移行を選択した場合は、クラスター化されたファイルサーバーリソースの名前を指定します。
...
3. **[カットオーバーの構成]** ページで、各ソースデバイスの設定を引き継ぐネットワークアダプターを指定します。 これにより、移行元から移行先に IP アドレスが移動されます。 すべてのネットワーク移行または特定のインターフェイスをスキップするオプションがあります。 サーバーをダウンさせる場合は、ソースインターフェイスに DHCP または新しい静的 IP アドレスを常に指定する必要があります。
...
5. 移行先サーバーの名前を変更した後、移行元サーバーの名前を変更する方法を指定します。 ランダムに生成された名前を使用することも、自分で入力することもできます。 その後、 **[次へ]** を選択します。

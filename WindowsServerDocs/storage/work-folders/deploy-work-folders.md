...
> [!NOTE]
>  これらのポリシー設定は、Windows 8.1 または Windows Server 2012 R2 以降でグループ ポリシーの管理を実行しているコンピューターからグループ ポリシーを編集する場合にのみ利用できます。 以前のオペレーティング システムに含まれるバージョンのグループ ポリシーの管理では、この設定は利用できません。 これらのポリシー設定は、[Windows 7 のワーク フォルダー](https://blogs.technet.com/b/filecab/archive/2014/04/24/work-folders-for-windows-7.aspx) アプリがインストールされている Windows 7 PC に適用されます。  
  
##  <a name="BKMK_LINKS"></a> 参照  

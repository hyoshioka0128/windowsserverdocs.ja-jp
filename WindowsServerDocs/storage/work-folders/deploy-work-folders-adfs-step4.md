...
「手順 1: AD FS のセットアップ」と「手順 3: ワーク フォルダーのセットアップ」で作成した、AD FS とワーク フォルダーの証明書を、Web アプリケーション プロキシの役割をインストールするローカル コンピューターの証明書ストアにインストールする必要があります。  
...
8.  **[公開設定]** ページで、以下を入力して、**[次へ]** をクリックします。  
    ...
    バックエンド サーバー URL: **https://workfolders.contoso.com**  

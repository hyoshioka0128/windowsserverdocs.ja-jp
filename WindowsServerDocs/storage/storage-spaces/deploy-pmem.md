...
PMem は、DRAM の速度を備えた非揮発性 DRAM (NVDIMM) の一種であり、そのメモリのコンテンツは電力サイクルを通じて保持されます (予期しない電源切れが発生した場合、ユーザーが開始したシャットダウン、システムクラッシュ、など)。 このため、中断した場所から再開すると、RAM のコンテンツを再読み込みする必要がなくなるため、大幅に高速になります。 もう1つの固有の特性は、PMem がバイトアドレス指定可能であることです。これは、これをストレージとしても使用できることを意味します (これは、PMem がストレージクラスのメモリとして参照されている可能性があるためです)。
...
次の表は、Windows Server 2019 および Windows Server 2016 でサポートされている永続メモリハードウェアを示しています。 Intel Optane は、特にメモリモードとアプリダイレクトモードの両方をサポートしていることに注意してください。 Windows Server 2019 は、混合モードの操作をサポートしています。

| 永続メモリテクノロジ                                      | Windows Server 2016 | Windows Server 2019 |
|-------------------------------------------------------------------|--------------------------|--------------------------|
| アプリダイレクトモードの**NVDIMM**                                       | Supported                | Supported                |
| アプリダイレクトモードでの**Intel Optane™ DC 永続メモリ**             | サポート非対象            | Supported                |
| **Intel Optane™** 2 レベルメモリモードの DC 永続メモリ (2lm) | サポート非対象            | Supported                |
...
NVDIMM が standard DIMM (メモリ) スロットに置かれ、プロセッサの近くにデータが配置されていることを思い出してください (そのため、待機時間を短縮し、パフォーマンスを向上させることができます)。 これを基にして、インターリーブセットは、2つ以上の NVDIMMs が N 方向インターリーブセットを作成し、スループットを向上させるためにストライプの読み取り/書き込み操作を行う場合に使用します。 最も一般的な設定は、2方向または4方向のインターリーブです。

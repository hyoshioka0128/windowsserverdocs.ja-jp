...
入れ子になったミラーアクセラレータのパリティを使用するには、`NestedMirror` と @no__t 層の両方のテンプレートを参照し、ボリュームの各部分に1つずつ、2つのサイズを指定します (ミラーの最初、パリティ 2)。 たとえば、20% の入れ子になった双方向ミラーと 80% の入れ子になったパリティである 1 500 GB ボリュームを作成するには、次のように実行します。

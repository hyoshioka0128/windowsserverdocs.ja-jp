...
description: 記憶域スペース ダイレクトの Windows Admin Center と PowerShell を使用してボリュームを作成する方法。
...
Windows Admin Center で 3 方向ミラー ボリュームを作成します。 

1. Windows Admin Center で、記憶域スペース ダイレクト クラスターに接続し、**ボリューム**から、**ツール**ウィンドウ。
2. [ボリューム] ページで、選択、**インベントリ**、タブを選び**ボリュームを作成する**します。
3. **ボリューム作成**、ボリュームの名前を入力して、ウィンドウで、**回復性**として**3 方向ミラー**します。
4. **サイズで、HDD**ボリュームのサイズを指定します。 たとえば、5 TB (テラバイト) です。
5. **[作成]** を選択します。

サイズによっては、ボリュームを作成する数分がかかることができます。 右上での通知を使用すると、ボリュームが作成されたときを把握できます。 新しいボリュームは、インベントリの一覧が表示されます。

3 方向ミラー ボリュームを作成する方法の簡単なビデオをご覧ください。

> [!VIDEO https://www.youtube-nocookie.com/embed/o66etKq70N8]

## <a name="create-a-mirror-accelerated-parity-volume"></a>ミラー アクセラレータを使用したパリティ ボリュームを作成します。

ミラー アクセラレータを使用したパリティ、HDD 上のボリュームのフット プリントが減少します。 たとえば、3 方向ミラー ボリュームはサイズの 10 テラバイトごとの場合が必要になる 30 テラバイト フット プリントとして。 フット プリントでオーバーヘッドを削減するには、ミラー アクセラレータを使用したパリティ ボリュームを作成します。 この、フット プリントを縮小 30 テラバイトからだけ 22 テラバイトまで、4 つのサーバーを使用しても、データの最もアクティブな 20% をミラーリングしてより多くの領域が残りの部分を格納する、効率的なは、パリティを使用します。 この比率のパリティとミラーのワークロードに対して適切なトレードオフを容量とパフォーマンスを調整できます。 たとえば、90% のパリティと 10% のミラーは小さいパフォーマンスが得られますが、フット プリントのさらに効率化します。

Windows Admin Center でミラー アクセラレータを使用したパリティ ボリュームを作成します。

1. Windows Admin Center で、記憶域スペース ダイレクト クラスターに接続し、**ボリューム**から、**ツール**ウィンドウ。
2. [ボリューム] ページで、選択、**インベントリ**、タブを選び**ボリュームを作成する**します。
3. **ボリューム作成**ウィンドウで、ボリュームの名前を入力します。
4. **回復性**、**ミラー アクセラレータを使用したパリティ**します。
5. **パリティ割合**パリティの割合を選択します。
6. **[作成]** を選択します。

ミラー アクセラレータを使用したパリティ ボリュームを作成する方法の簡単なビデオをご覧ください。

> [!VIDEO https://www.youtube-nocookie.com/embed/R72QHudqWpE]

## <a name="open-volume-and-add-files"></a>ボリュームを開き、ファイルの追加

ボリュームを開き、Windows Admin Center でボリュームにファイルを追加します。

1. Windows Admin Center で、記憶域スペース ダイレクト クラスターに接続し、**ボリューム**から、**ツール**ウィンドウ。
2. [ボリューム] ページで、選択、**インベントリ**タブ。
2. ボリュームの一覧で、開きたいボリュームの名前を選択します。

    ボリュームの詳細 ページで、ボリュームへのパスを確認できます。

4. ページの上部にある次のように選択します。**オープン**します。 これは、Windows Admin Center で、ファイル ツールを起動します。
5. ボリュームのパスに移動します。 ここで、ボリューム内のファイルを参照できます。
6. 選択**アップロード**、しアップロードするファイルを選択します。
7. ブラウザーを使用して**戻る**Windows Admin Center でツール ウィンドウに戻るボタンをクリックします。

ボリュームを開き、ファイルを追加する方法の簡単なビデオをご覧ください。

> [!VIDEO https://www.youtube-nocookie.com/embed/j59z7ulohs4]

## <a name="turn-on-deduplication-and-compression"></a>重複除去と圧縮を有効にします。

重複除去と圧縮は、ボリューム別に管理されます。 重複除去と圧縮は、節約は、それが実行されるまで表示されませんが、処理後のモデルを使用します。 この場合は前に、からありましたいる場合でも、すべてのファイルを操作します。

1. Windows Admin Center で、記憶域スペース ダイレクト クラスターに接続し、**ボリューム**から、**ツール**ウィンドウ。
2. [ボリューム] ページで、選択、**インベントリ**タブ。
3. ボリュームの一覧では、管理するボリュームの名前を選択します。
4. ボリュームの詳細 ページで、ラベル付け、スイッチをクリックします。**重複除去と圧縮**します。
5. 有効にする重複除去ウィンドウで、重複除去モードを選択します。

    複雑な設定は、代わりに Windows Admin Center ではさまざまなワークロード用の既製のプロファイルを選択できます。 不明な場合は、既定の設定を使用します。

6. **[有効化]** を選びます。

重複除去と圧縮を有効にする方法の簡単なビデオをご覧ください。

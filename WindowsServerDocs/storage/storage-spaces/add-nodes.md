...
![4 つのノードのクラスターにサーバーを追加するアニメーション](media/add-nodes/Scaling-Out.gif)
...
![3 番目のサーバーを 2 つのノードのクラスターに追加します。](media/add-nodes/Scaling-2-to-3.png)
...
![4 番目のサーバーを 3 つのノードのクラスターに追加します。](media/add-nodes/Scaling-3-to-4.png)
...
![システム ドライブを追加することを示すアニメーション](media/add-nodes/Scale-Up.gif)
...
## ドライブやサーバーを追加した後、ドライブの使用率を最適化します。

時間の経過と共にドライブが追加または削除されると、プール内のドライブの間でのデータの配布なりますとともに。 場合によっては、この結果、特定のドライブがいっぱいに、プール内の他のドライブがある程度の消費の削減。

プール間であっても、ドライブの割り当てを保つためには、記憶域スペース ダイレクトが自動的に最適化ドライブの使用率 (これは、共有 SAS エンクロージャを使用した記憶域スペースのシステムを手動で行うプロセス) プールにドライブやサーバーを追加した後です。 最適化は、プールに新しいドライブを追加した後、15 分を開始します。 プールの最適化は、かかることが時間、または日を完了するには大きなハード ディスク ドライブを使っている場合に特にため、優先順位の低いバック グラウンド操作として実行されます。

最適化が 2 つのジョブと呼ばれる 1 つの*最適化*と 1 つと呼ばれる*再調整*- を使用し、次のコマンドを使用して、進行状況を監視することができます。

```powershell
Get-StorageJob
```

手動でのコマンドレットが[Optimize-storagepool](https://docs.microsoft.com/powershell/module/storage/optimize-storagepool?view=win10-ps)記憶域プールを最適化できます。 以下に例を示します。

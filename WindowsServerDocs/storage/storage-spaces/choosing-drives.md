...
ms.locfileid: "7871595"
---
# 記憶域スペース ダイレクト用のドライブの選択
...
1. **すべて NVMe。** すべて NVMe の展開を使用すると、優れたパフォーマンスが実現され、予測可能な待機時間が最も短くなります。 すべてのドライブが同じモデルである場合、キャッシュはありません。 耐久性が高い NVMe モデルと耐久性が低い NVMe モデルを組み合わせ、耐久性が高い NVMe モデルを、耐久性が低い NVMe モデルに対する書き込みをキャッシュするように構成することもできます ([セットアップが必要です](understand-the-cache.md#manual))。
...
3. **すべて SSD。** すべて NVMe の展開と同様に、すべてのドライブが同じモデルである場合は、キャッシュはありません。 耐久性が高いモデルと耐久性が低いモデルを組み合わせた場合、耐久性が高い NVMe モデルを、耐久性が低い NVMe モデルに対する書き込みをキャッシュするように構成することができます ([セットアップが必要です](understand-the-cache.md#manual))。
...
![利用可能なハイブリッド展開](media/choosing-drives-and-resiliency-types/Hybrid-Deployment-Possibilities.png)
...
>[!IMPORTANT]
>のみ Hdd で構成はサポートされていません。 耐久性の高い Ssd 耐久性が低い Ssd のキャッシュがお勧めできません。
...
キャッシュは、アプリケーションやつまりすべてのデータが読み取りや書き込み任意の時点では、ワークロードのワーキング セットに合わせてサイズ変更する必要があります。 キャッシュのサイズについては、それ以外の要件はありません。 Hdd で展開では、公正な出発点は – 容量の 10% など、各サーバーが 4 x 4 TB の HDD 16 tb 合計容量は 2 800 GB SSD し、サーバーあたりのキャッシュ 1.6 tb になります。 オール フラッシュ展開では、特に、非常に[高い耐久性](https://blogs.technet.microsoft.com/filecab/2017/08/11/understanding-dwpd-tbw/)Ssd には、たとえば – 容量の 5% に近いを開始する公正は、各サーバーに 1.2 TB SSD 24 = 合計容量は、2 倍の 750 GB NVMe 時に可能性がありますサーバーあたりのキャッシュ 1.5 tb になります。 キャッシュ ドライブは、いつでも追加または取り外して、後で調整することができます。

...
リストを表示するには、コロンを使用します。 以下に例を示します。
...
### <a name="2-show-dont-tell"></a>2. 表示、わかりません

テキストは可能な限りシンプルにします (小さなアニメーションやビデオを検討してください)。

### <a name="3-dont-overwhelm"></a>3.過負荷にならない
...
### <a name="4-keep-it-contextual"></a>4。コンテキストを維持する
...
### <a name="5-minimize-painful-setup"></a>5。設定の手間を最小限にする
...
- タスクのタイトルと同じテキストを使用する必要はありません。UI のコンテキストで意味のあるテキストを使用しますが、2つの間に論理接続があることを確認してください。 以下に例を示します。 
...
|処理中         |実行している操作の動詞から開始し、省略記号で終了して進行中の操作を示します。 次に例を示します。<br> *ボリューム "顧客データ" を作成しています...*|
|成功             |"正常に" で開始し、ソフトウェアが実行した内容で終了します。 次に例を示します。<br> *ボリューム "Customer data" が正常に作成されました。*|
|失敗             |"できませんでした" で終了し、ソフトウェアで実行できなかった内容を含めます。 次に例を示します。<br> *ボリューム "顧客データ" を作成できませんでした。*|
...
|    コンテキスト                                 |    ヒントを記述する方法    |
|    -----------------------                 |    -------------------------    |
|コントロールまたは UI 要素のラベルが付けられていない場合...|単純でわかりやすい名詞句を使用します。 以下に例を示します。<br> ペンの強調表示 |
...
|テキストラベルが切り詰められているか、一部の言語で切り捨てられる可能性があります...|<ul><li>ツールヒントで、切り捨てられていないラベルを指定します。</li><li>省略可能: 別の行で、明確な説明を指定しますが、必要な場合にのみ指定します。</li><li>切り捨てられていない情報がページまたはフローの別の場所にある場合は、ツールヒントを指定しないでください。</li></ul>|
|キーボードショートカットを使用できる場合は、|<ul><li>省略可能: ラベルまたは説明の語句の後に、キーボードショートカットをかっこで囲んで指定します。例:"印刷 (Ctrl + P)" または "このファイル内のテキストの検索 (Ctrl + F)"</li><li>明確なヒントに便利なショートカットキーを追加しても問題はありませんが、キーボードショートカットを表示するためだけにツールヒントを追加することは避けてください。 </li></ul>|
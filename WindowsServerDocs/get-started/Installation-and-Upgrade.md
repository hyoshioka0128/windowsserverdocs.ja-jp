...
ms.locfileid: "6121481"
---
# Windows Server のインストールとアップグレード

>適用対象: Windows Server 2016、Windows Server 2012 R2、Windows Server 2012、Windows Server 2008 R2、Windows Server 2008

> [!IMPORTANT]
> Windows Server 2008 R2 と Windows Server 2008 の延長サポートは 2020 年 1月で終了します。 [アップグレード オプションについて説明します](#upgrading-from-windows-server-2008-r2-or-windows-server-2008)。
...
## 移行 (Windows Server 2016 への移行で推奨される手順)
...
クラスター OS のローリング アップグレードは Windows Server 2016 の新機能です。管理者はこの機能を利用して、Hyper-V やスケールアウト ファイル サーバーのワークロードを停止することなく、クラスター ノードのオペレーティング システムを Windows Server 2012 R2 から Windows Server 2016 にアップグレードできます。 この機能により、サービス レベル アグリーメントに影響する可能性のあるダウンタイムが回避できます。 この新機能について詳しくは、「[クラスター オペレーティング システムのアップグレードを公開](https://technet.microsoft.com/windows-server-docs/failover-clustering/cluster-operating-system-rolling-upgrade)」をご覧ください。
...
|-------------------------|---------------------------|
|Windows Server2008R2 Datacenter sp1|Windows Server 2012 R2 Datacenter|
|Windows Server2008R2 Enterprise sp1|Windows Server 2012 R2 Standard または Windows Server 2012 R2 Datacenter|
|Windows Server2008R2 Standard sp1|Windows Server 2012 R2 Standard または Windows Server 2012 R2 Datacenter|
|Windows の Web Server2008R2 sp1|Windows Server 2012 R2 Standard|
...
|Windows Server2008R2 Standard SP1 または Windows Server2008R2 Enterprise sp1|Windows Server 2012 Standard、Windows Server 2012 Datacenter|
|Windows Server2008R2 Datacenter sp1|Windows Server 2012 Datacenter|
...
## Windows Server 2008 R2 または Windows Server 2008 からのアップグレード

前述のように、 [Windows Server 2008 のアップグレードおよび Windows Server 2008 R2](modernize-windows-server-2008.md)、Windows Server 2008 R2/Windows Server 2008 の延長サポートは 2020 年 1 月の終了します。 ギャップをサポートしないようにするため、Windows Server のサポートされているバージョンにアップグレードするか、[専用の Windows Server 2008 R2 Vm](uploading-specialized-WS08-image-to-azure.md)に移動して、Azure で残して書再配置する必要があります。 移行/アップグレードの計画に関する考慮事項については、 [Windows Server の移行ガイド](https://go.microsoft.com/fwlink/?linkid=872689)を確認します。

オンプレミスでは、サーバーは Windows Server 2016 以降、Windows Server 2008 R2 から直接アップグレード パスはありません。 代わりに、Windows Server 2012 R2、およびし、 [Windows Server 2016 へのアップグレード](#Upgrading-to-Windows-Server-2016)を最初にアップグレードします。

アップグレードを計画しているように、Windows Server 2012 R2 へのアップグレードの中間の手順に関するガイドラインは次の注意してください。

  - アーキテクチャ 64 ビットと 32 ビット アーキテクチャからでは、インプレース アップグレードを行うか、いずれかからビルドの種類別 (たとえば、chk を fre) をことはできません。

  - 一括アップグレードは、同じ言語でのみサポートされます。 1 つの言語から別にアップグレードすることはできません。

  - Windows Server 2008 の server core インストールから (Windows Server では「サーバーの完全なデスクトップ」と呼ばれます) サーバーの GUI を備えた Windows Server 2012 R2 に移行することはできません。 Windows Server 2012 R2 でのみが、完全なデスクトップ、サーバーには、アップグレード後の server core インストールを切り替えることができます。 Windows Server 2016 とそれ以降の*しない*サポートが server core から完全なデスクトップに切り替えるようにそのスイッチを Windows Server 2016 にアップグレードする前にします。
  
詳細については、チェック アウト[アップグレード オプションの Windows Server 2012 の評価版と](https://docs.microsoft.com/previous-versions/windows/it-pro/windows-server-2012-R2-and-2012/jj574204\(v=ws.11\))特定の役割のアップグレードの詳細が含まれています。

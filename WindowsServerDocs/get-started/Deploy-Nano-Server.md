...
9. 管理者特権でのコマンド プロンプトで、Nano Server VHD があるディレクトリに移動し、次のコマンドを実行します。**md mountdir**

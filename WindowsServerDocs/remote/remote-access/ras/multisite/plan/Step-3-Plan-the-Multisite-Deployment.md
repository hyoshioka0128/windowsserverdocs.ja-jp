...
## <a name="bkmk_3_1_IPHTTPS"></a>3.1 IP-HTTPS 証明書の計画  
...
## <a name="bkmk_3_2_NLS"></a>3.2 ネットワークロケーションサーバーを計画する  
...
## <a name="bkmk_3_3_IPsec"></a>3.3 すべてのリモートアクセスサーバーの IPsec ルート証明書を計画する  
...
## <a name="bkmk_3_4_GSLB"></a>3.4 グローバルサーバーの負荷分散を計画する  
...
## <a name="bkmk_3_5_EP_Selection"></a>3.5 DirectAccess クライアントのエントリポイントの選択を計画する  
...
## <a name="bkmk_3_6_IPv6"></a>3.6 プレフィックスとルーティングを計画する  
...
## <a name="bkmk_3_7_TransitionIPv6"></a>3.7 マルチサイトのリモートアクセスが展開されている場合の IPv6 への移行を計画する  
...
### <a name="TransitionIPv4toMixed"></a>IPv4 専用から IPv6 + IPv4 企業ネットワークへの移行  
...
### <a name="TransitionMixedtoIPv6"></a>IPv6 + IPv4 から IPv6 専用の企業ネットワークへの移行  
...
### <a name="DualDeployment"></a>デュアル DirectAccess 展開を使用した IPv4 のみの展開から IPv6 のみの展開への移行  
...
1.  2番目の DirectAccess 展開をインストールします。 新しいサーバーに DirectAccess をインストールするか、最初の展開からサーバーを削除して、2つ目の展開で使用することができます。  
    ...
    >   
    > はじめにウィザードまたはコマンド @no__t レットを使用して DirectAccess をインストールすると、リモートアクセスによって、展開内の最初のエントリポイントのクライアントプレフィックスが < IPv6 サブネット @ no__t-1 prefix >: 1000::/64 の既定値に自動的に設定されます。 必要に応じて、プレフィックスを変更する必要があります。  

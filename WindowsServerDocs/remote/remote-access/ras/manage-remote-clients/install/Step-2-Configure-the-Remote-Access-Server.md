...
5.  **Network Connectivity Assistant** ページ。  
...
2.  インフラストラクチャ サーバーのセットアップ ウィザードでの **ネットワーク ロケーション サーバー**  ページで、展開でネットワーク ロケーション サーバーの場所に対応するオプションをクリックします。  
...
3.  **DNS**  ページで、テーブルに追加の名前サフィックスを入力適用される名前解決ポリシー テーブル (NRPT) 除外として。 ローカル名の解決オプションを選択し、クリックして **次**します。  
  
4.  **DNS サフィックス検索一覧**  ページで、リモート アクセス サーバーが、展開内のドメイン サフィックスを自動的に検出します。 使用して、 **追加** と **削除** ボタンを使用するドメイン サフィックスの一覧を作成します。 新しいドメイン サフィックスを追加する **新しいサフィックス**, をサフィックスを入力して、クリックして **追加**します。 **[次へ]** をクリックします。  

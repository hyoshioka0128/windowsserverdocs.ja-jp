...
> [!NOTE]  
> これらのタスクを続行する前に、「[Step 1:リモートアクセスインフラストラクチャ @ no__t-0 を計画します。  
...
-   **ネットワークトポロジ**  
    ...
    -   **2 つのアダプター**:2つのネットワークアダプターを使用する場合、リモートアクセスは、インターネットに直接接続された1つのネットワークアダプターと内部ネットワークに接続されたネットワークアダプターを使用して構成できます。 または、サーバーが、ファイアウォールやルーターなどのエッジデバイスの背後に設置されている場合もあります。 この構成では、1つのネットワークアダプターが境界ネットワークに接続され、もう一方が内部ネットワークに接続されます。  
...
-   **\[認証]**  
    ...
    -   **コンピューターの認証**:証明書を使用するようにコンピューター認証を構成できます。 または、リモートアクセスサーバーは、証明書を要求することなく、Kerberos 認証のプロキシとして機能できます。 

...
スプリット ブレイン DNS 環境で使用できるリソースの両方のバージョンを実行する場合は、インターネット上で使用される名前と重複しないように名前を持つ、イントラネット リソースを構成します。 ユーザーがイントラネット上のリソースにアクセスするときに代替名を使用するように指示します。 たとえば、www を構成\.www の内部名の internal.contoso.com\.contoso.com です。  

...
[ビデオハードウェア](#selecting-video-hardware) 
 の選択[ダイレクト \- ビデオ \- 接続または USB ゼロクライアントステーション](#BKMK_Selectingdirect-video-connectedorUSBzeroclientstationdevices)の選択デバイス 
 [他のステーション周辺](#selecting-other-station-peripheral-devices)機器を選択する 
 [ \- LAN 経由で \- \- 接続ステーションのハードウェア](#BKMK_SelectingRDP-over-LAN-connectedstationhardware)を選択する 
 [オーディオデバイスを選択](#selecting-audio-devices)する
...
## <a name="selecting-direct-video-connected-or-usb-zero-client-station-devices"></a><a name="BKMK_Selectingdirect-video-connectedorUSBzeroclientstationdevices"></a>直接 \- ビデオ \- 接続または USB ゼロクライアントステーションデバイスを選択する
...
- [USB ハブを選択します。](#selecting-usb-hubs)
...
属さない限り、各 MultiPoint サービスのラジオ局を提供する 1 つのモニターになければなりません分割\-画面を計画します。 モニターは、コンピューターのビデオカード、USB ゼロクライアント、または LAN ベースのクライアントに接続されてい \- ます。 任意の種類のビデオ カード、USB ゼロ クライアント、または LAN でサポートされているモニター\-CRT モニターを含むベースのクライアントを使用できます。
...
| 説明 | ホスト コンピューターに接続されています。 | ステーション ハブまたはダウン ストリーム ハブに接続されています。 |
|--|--|--|
...
| HID コンシューマー コントロール | 動作しない | 個々 のステーションに利用可能です |
...
## <a name="selecting-rdp-over-lan-connected-station-hardware"></a><a name="BKMK_SelectingRDP-over-LAN-connectedstationhardware"></a>\- \- LAN \- 接続ステーションのハードウェアで RDP を選択する
...
\/ \- \- コンピューターのマザーボードでキーボードとマウスの ps 2 ポートを使用して ps 2 ダイレクトビデオ接続ステーションを構成した場合 \/ 、MultiPoint Services システムがステーションモードで実行されているときに、オーディオデバイスをこのステーションで使用できるようにするには、コンピューターのマザーボードでアナログオーディオを使用する必要があります。

PS \/ 2 ダイレクトビデオ接続ステーションがない場合 \- \- 、システムのマザーボード上のホストオーディオデバイスは、MultiPoint Services システムがコンソールモードで実行されている場合にのみ使用できます。
...
| devicestoredirect:s:value | USB デバイスのリダイレクト:</br>ローカル コンピューター上のどのデバイスがリダイレクトされて、リモート セッションで使用可能になるかを決定します。 | - *:後で接続されるものも含め、サポートされているすべてのデバイスをリダイレクトします</br> - 1 つまたは複数のデバイスの有効なハードウェア ID | デバイスをリダイレクトしない | はい |
...
| RDP 設定                        | 説明            | 値                 | 既定値          | Windows Virtual Desktop サポート |
|------------------------------------|------------------------|------------------------|:----------------------:|:-----------------------:|
| use multimon:i:value | リモート セッションでローカル コンピューターからの 1 つまたは複数の表示を使用するかどうかを決定します。 | - 0:複数ディスプレイのサポートを有効にしない</br>- 1:複数ディスプレイのサポートを有効にする | 0 | はい |
...
| screen mode id:i:value | 接続を起動したときに、リモート セッション ウィンドウが全画面表示されるかどうかを指定します。 | - 1:リモート セッションはウィンドウで表示される</br>- 2:リモート セッションは全画面で表示される | 2 で保護されたプロセスとして起動されました | はい |

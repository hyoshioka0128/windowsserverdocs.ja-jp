...
Microsoft は毎月第2火曜日にセキュリティ情報をリリースしますが、まれなことですが、セキュリティ更新プログラムは、毎月のセキュリティ更新プログラム ("帯域外" 更新プログラムとも呼ばれます) の間で、脆弱性が検出されたときにリリースされます。顧客システムへの緊急リスク。 小規模企業で、システムとアプリケーションの修正プログラムを管理するために Windows Update を使用するように Windows コンピューターを構成するか、大規模な組織で Microsoft Endpoint Configuration Manager などの管理ソフトウェアを使用して、修正プログラムを展開します。階層型の計画では、多くのお客様が比較的タイムリーに Windows インフラストラクチャを修正します。  

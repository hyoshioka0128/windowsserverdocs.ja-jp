...
ここに示されている SCM のベースラインの推奨事項と、セキュリティの侵害を検出するために推奨される設定は、管理者にとって最初のベースラインガイドにすぎません。 各組織は、直面する脅威、許容されるリスク許容範囲、および有効にする必要がある監査ポリシーカテゴリまたはサブカテゴリについて、独自の決定を行う必要があります。 脅威の詳細については、[脅威と対策](https://technet.microsoft.com/library/hh125921(v=ws.10).aspx)に関するガイドを参照してください。 適切な監査ポリシーが設定されていない管理者は、運用環境でを実装する前に、ここで推奨されている設定から開始し、変更してテストすることをお勧めします。  
...
> [!NOTE]  
> Microsoft Windows の既定値とベースラインの推奨事項は、 [Microsoft セキュリティコンプライアンスマネージャーツール](https://technet.microsoft.com/library/cc677002.aspx)から取得されました。  
...
|監査ポリシーカテゴリまたはサブカテゴリ|Windows の既定値<br /><br />成功の失敗|ベースラインの推奨事項<br /><br />成功の失敗|より強力な推奨事項<br /><br />成功の失敗|  
|----------------------------------------|------------------------------------------|--------------------------------------------------|--------------------------------------------------|  
|**アカウントのログオン**||||  
...
侵害の兆候を監視するときに含める必要があるイベントの包括的な一覧については、[Appendix L:監視するイベント @ no__t-0。  
...
@No__t を参照してください-0Appendix L:監視する推奨イベントの一覧、その重要度の評価、およびイベントメッセージの概要を監視するための、@ no__t-0 を監視するイベント。  
...
**高い**重要度が高いイベント Id は、常に、直ちに警告と調査を行う必要があります。  

**程度**中程度の重要度の評価を持つイベント ID は、悪意のあるアクティビティを示している可能性がありますが、他の異常が (たとえば、特定の期間に発生した異常な数値、予期しない数、またはコンピューター上で発生した異常な数値など) を伴う可能性があります。通常、イベントをログに記録する必要はありません)。 中程度の重要度のイベントは、メトリックとして収集し、時間の経過と共に比較することもできます。  

**低画質**重要度の低いイベントを含むイベント ID は、中レベルまたは高重要度のイベントに関連付けられている場合を除き、収集したり、アラートを発生させたりしないでください。  

これらの推奨事項は、管理者のベースラインガイドを提供することを目的としています。 すべての推奨事項は、運用環境で実装する前に十分に確認してください。  

@No__t を参照してください-0Appendix L:監視する推奨イベントの一覧、重要度の評価、およびイベントメッセージの概要については、@ no__t-0 を監視するイベント。  

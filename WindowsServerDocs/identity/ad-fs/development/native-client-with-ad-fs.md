...
このプロジェクトでは、Web.config と Startup.Auth.cs の2つのファイルに変更が必要です。 パラメーターの正しい値を取得するには、Web.config の変更が必要です。 Azure AD ではなく AD FS に対して認証するように WebAPI を設定するには、Startup.Auth.cs の変更が必要です。

---
title: AD FS 2016 で OAuth パブリック クライアントを使用してネイティブ クライアント アプリケーションを構築します。
description: OAuth パブリック クライアントと AD FS 2016 を使用してネイティブ クライアント アプリケーションの構築方法について説明するチュートリアル
author: billmath
ms.author: billmath
ms.reviewer: anandy
manager: mtillman
...
ms.locfileid: "59846483"
---
# <a name="build-a-native-client-application-using-oauth-public-clients-with-ad-fs-2016"></a>AD FS 2016 で OAuth パブリック クライアントを使用してネイティブ クライアント アプリケーションを構築します。

## <a name="overview"></a>概要

この記事では、AD FS 2016 で保護された Web API と連携するネイティブ アプリケーションを作成する方法を示します。
...
2. アクセス トークンは、ベアラー トークンとして TodoListService web API の/todolist エンドポイントを呼び出すときにユーザーの認証に使用されます。
 私たちは、Azure AD は、ここの例では、アプリケーションを使用してはし、AD FS 2016 に変更します。
...
* キーを追加**ida: 機関**広告を示す値を持つ FS サービス、例では、 https://fs.contoso.com/adfs/
* 変更**ida: ClientId**キーから値を**ネイティブ アプリケーション**AD FS でアプリケーション グループの作成中にページ。 Ex、3f07368b-6efd-4f50-a330-d93853f4c855
* 変更、 **todo:TodoListBaseAddress** Web API のベース アドレスを例。 https://localhost:44321/
* 値を設定**ida: RedirectUri**を AD FS でアプリケーション グループの追加中に、ネイティブ アプリケーションのページに配置する値。
* 読みやすさの削除/コメントのキー **ida: テナント**と**ida: AADInstance**します。

**MainWindow.xaml.cs**

* AadInstance の行を削除します
* 次に示すように機関の値を追加します。
...
* 関数で**MainWindow**、として authContext インスタンス化の変更

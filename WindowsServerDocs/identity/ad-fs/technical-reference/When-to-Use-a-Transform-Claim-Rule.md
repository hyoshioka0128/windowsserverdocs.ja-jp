...
ms.locfileid: "71385419"
---
# <a name="when-to-use-a-transform-claim-rule"></a>変換要求規則を使用するタイミング
入力方向の要求の種類を出力方向の要求の種類にマップする必要がある場合に、この規則を Active Directory フェデレーションサービス (AD FS) \(AD FS @ no__t-1 で使用することができます。その後、次の値に基づいて発生する出力を決定するアクションを適用します。入力方向の要求。 この規則を使用する場合は、次の規則のロジックと一致する要求を、次の表で説明する規則の構成オプションのいずれかに基づいてパススルーまたは変換します。  
...
|受信 e @ no__t-0mail サフィックスの要求を新しい e @ no__t-1mail サフィックスに置き換える|入力方向の要求の種類が "指定された要求の種類" に等しく、値が "任意のサフィックスの値" に等しい場合は、新しい出力方向の要求の値 "指定されたサフィックスの値" と出力要求の種類 "指定された要求の種類" で要求を変換します|  
...
この規則を使用すると、指定した e @ no__t-0mail サフィックスの値を持つすべての入力方向の要求を新しい値に置き換える条件を適用することもできます。 たとえば、sales.corp.fabrikam.com のサフィックスを持つすべての要求の値を fabrikam.com に変更する条件を、この規則で設定できます。  
...
この規則を作成するには、要求規則言語を使用するか、AD FS Management snap @ no__t から、"**入力方向の要求の変換**" 規則テンプレートを使用します。 この規則テンプレートには、次の構成オプションがあります。  
...
-   受信 e @ no__t-0mail サフィックスの要求を新しい e @ no__t-1mail サフィックスに置き換えます。  
...
正規表現を使用して、要求を変換することもできます。 たとえば、次の規則は、ドメイン @ no__t-0USER 形式の windows ユーザー名要求のドメインを FABRIKAM に設定します。  

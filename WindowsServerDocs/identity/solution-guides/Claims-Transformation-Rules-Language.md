...
**Active Directory 用の Windows PowerShell コマンドレット**:これは、要求変換ポリシーを作成および設定するための推奨される方法です。 これらのコマンドレットは、単純なポリシーのスイッチを提供し、より複雑なポリシーに設定されている規則を検証します。  
  
**LDAP**:要求変換ポリシーは、ライトウェイトディレクトリアクセスプロトコル (LDAP) を使用して Active Directory で編集できます。 ただし、これは推奨されません。ポリシーには複雑なコンポーネントがいくつかあり、使用するツールがポリシーを検証せずに Active Directory に書き込むことができるためです。 その後、問題を診断するためにかなりの時間が必要になることがあります。  
...
-   要求変換規則セットは、0個以上の規則で構成されています。 各ルールには、次の2つのアクティブな部分があります。**条件リスト**と**ルールアクション**を選択します。 **[条件の選択] の一覧**が TRUE と評価された場合、対応するルールアクションが実行されます。  
...
2.  動作している**要求セット**:要求変換中に読み取りおよび書き込みを行う中間要求。  
...
1. 例:  
   ...
   *Line number:1、列番号:2、エラートークン:、。行: ' c1;[] = > 問題 (要求 = c1); '。*  
   *Parser エラー:'POLICY0030:構文エラーです。予期しない '; ' が発生しています。次のいずれかが必要です: ': '。 '*  
...
3. 例:  
   ...
   *Parser エラー:'POLICY0030:構文エラーです。予期しない ' STRING ' が指定されています。次のいずれかが必要です:' INT64_TYPE ' ' UINT64_TYPE ' ' STRING_TYPE ' ' BOOLEAN_TYPE ' ' IDENTIFIER '*  
...
4. 例:  
   ...
   **エラー メッセージ:**  
   *POLICY0002:ポリシーデータを解析できませんでした。*  
   *Line number:1、列番号:23、エラートークン:1. 行: ' c1: [type = = "x1", value = = 1, valuetype = = "bool"] = > Issue (claim = c1); '。* @ no__t-1 パーサーエラー:'POLICY0029:予期しない入力です。 </em>  
...
5. 例:  
   ...
   **エラー メッセージ:**  
   *POLICY0002:ポリシーデータを解析できませんでした。*  
   *Line number:1、列番号:91、エラートークン: = =。行: ' c1: [type = = "x1", value = = "1",*  
   *valuetype = = "boolean"] = > Issue (type = c1. type, value = "0", valuetype = = "boolean"); '。*  
   *Parser エラー:'POLICY0030:構文エラーです。予期しない ' = = ' が想定されています: ' = '*  

...
|フェーズ|説明  
|---------|---------------  
|[プラン:ポリシーと展開に必要な構成の必要性を識別します。](Deploy-a-Central-Access-Policy--Demonstration-Steps-.md#BKMK_1.2)|ポリシーのニーズおよび展開に必要な構成を特定します。 
...
|[保持します。変更およびステージングするポリシー](Deploy-a-Central-Access-Policy--Demonstration-Steps-.md#BKMK_1.5)|ポリシーの変更およびステージングします。 

## <a name="BKMK_1.1"></a>テスト環境を設定します。  
開始する前に、このシナリオをテストするラボを設定する必要があります。 ラボを設定するための手順で詳しく説明は[付録 b:テスト環境のセットアップ](Appendix-B--Setting-Up-the-Test-Environment.md)します。  

## <a name="BKMK_1.2"></a>プラン:ポリシーのニーズおよび展開に必要な構成を特定する  
...
|1.3|Windows Server 2012 コンストラクトでアクセス ポリシーを表現します。|ターゲット:<br /><br />-Resource.Department には Finance が含まれています<br /><br />アクセス規則:<br /><br />-   Allow read User.Country=Resource.Country AND User.department = Resource.Department<br />-フル コントロール User.MemberOf(FinanceAdmin) を許可します。<br /><br />例外:<br /><br />Allow read memberOf(FinanceException)|  
|1.4|ポリシーに必要なファイル プロパティを決定する|次の項目でファイルにタグを付けます。<br /><br />部門<br />-国|  
|1.5|ポリシーに必要な要求の種類とグループを決定する|要求の種類:<br /><br />-国<br />部門<br /><br />ユーザー グループ:<br /><br />-FinanceAdmin<br />-FinanceException|  
|1.6|このポリシーを適用するサーバーを決定する|すべての金融ファイル サーバーでポリシーを適用します。|  

## <a name="BKMK_1.3"></a>実装:コンポーネントおよびポリシーを構成する  
...
1. パスワードを使用して contoso \administrator としてのログ、HYPER-V マネージャーでサーバー DC1 を開き <strong>pass@word1</strong>します。  
...
![ソリューション ガイド](media/Deploy-a-Central-Access-Policy--Demonstration-Steps-/PowerShellLogoSmall.gif)***<em>Windows PowerShell の同等のコマンド</em>***  
...
または、Windows Server 2012 コンストラクトでルールを表現します。  
...
1. Hyper-V マネージャーでサーバー FILE1 に接続します。 パスワードを使用して contoso \administrator を使用してサーバーにログオン:  <strong>pass@word1</strong>します。  
...
3. Active Directory からグローバル リソース プロパティーを更新する必要もあります。 管理者特権で Windows PowerShell ウィンドウを開き、「`Update-FSRMClassificationpropertyDefinition`」と入力します。 Enter キーをクリックしてから、Windows PowerShell を閉じます。  
   ...
   > 
   > 1. Contoso \administrator としてファイル サーバー FILE1 パスワードを使用してにログオン <strong>pass@word1</strong>します。  
...
1. パスワードを使用して contoso \administrator として上の HYPER-V マネージャーとログのファイル サーバー FILE1 に接続 <strong>pass@word1</strong>します。  

...
description: 記憶域、CPU、ネットワーク、メモリ、Windows Server 2019 のクリーン インストールの RAM の最小要件です。
ms.custom: na
...
ms.locfileid: "8976658"
---
# システム要件

>適用対象: Windows Server 2019 

このトピックでは Windows Server を実行するための最小システム要件について説明&reg;2019 します。

## システム要件を確認する。  
Windows Server 2019 の推定システム要件を次に示します。 使用しているコンピューターが "最小" 要件を満たしていない場合は、この製品を正しくインストールすることはできません。 実際の要件は、システム構成やインストールするアプリケーションおよび機能によって異なります。
...
- ECC (誤り訂正符号) 型または同様のテクノロジ、物理ホストの展開
...
Windows Server 2019 を実行しているコンピューターでは、PCI Express アーキテクチャの仕様に準拠している記憶域アダプターを含める必要があります。 ハード ディスク ドライブとして分類されるサーバー上の永続的な記憶装置は、PATA であってはなりません。 Windows Server 2019 では、ブート、ページ、またはデータ ドライブに ATA、PATA、IDE/EIDE は許可されません。  
...
サポート pre-boot Execution Environment (PXE) は有益である、ネットワーク アダプターが最小要件ではありません。

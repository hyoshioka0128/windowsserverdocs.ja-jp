...
-   Protected Users セキュリティ グループのメンバーの認証には、NTLM、ダイジェスト認証、および CredSSP 既定資格情報委任は使用できません。 これらのセキュリティ サポート プロバイダー (Ssp) のいずれかを使用して Windows 8.1 を実行しているデバイスでは、アカウントが Protected Users セキュリティ グループのメンバーである、ドメインに対する認証は失敗します。
...
2.  Windows Server 2012 R2 を実行しているドメインのドメイン コント ローラーは spn1 をサービスの Active Directory Domain Services アカウントを検索し、サービス チケット発行を制限する認証ポリシーを使用して、アカウントが構成されていることを決定します。

3.  ドメイン コント ローラーが TGT で、構成済みのアクセス制御条件と、ユーザーの id 情報を使用して、アクセス チェックを実行します アクセス検査の結果は不合格です。

4.  ドメイン コントローラーは要求を拒否します。

アカウントが許可されているため、認証ポリシーによって設定されたアクセス制御条件を満たしているし、TGT を持つユーザーが、サービスに接続しようとしています場合 (など、サービスに認証を必要とするアプリケーションを開くことである。サービスの SPN で識別) 次のシーケンスが発生します。

1.  SPN1 への接続を試行するために、TGS-REQ がドメイン コントローラーに送信されます。これは、SPN1 に対するサービス チケットを要求するものです。

2.  Windows Server 2012 R2 を実行しているドメインのドメイン コント ローラーは spn1 をサービスの Active Directory Domain Services アカウントを検索し、サービス チケット発行を制限する認証ポリシーを使用して、アカウントが構成されていることを決定します。

3.  ドメイン コント ローラーが TGT で、構成済みのアクセス制御条件と、ユーザーの id 情報を使用して、アクセス チェックを実行します アクセス検査の結果は合格です。

...
|**Max失敗したノード**|ほとんどのクラスターでは、クラスター ノードの数の約 3 分の 1 に相当する整数|ノードの障害またはクラスター サービスの実行停止を原因とした更新の失敗を許容できるノードの最大数。 この数を超えるノードで失敗すると、更新実行は停止します。<p> 有効な値の範囲は、0 からクラスター ノードの数より 1 少ない数までです。|  

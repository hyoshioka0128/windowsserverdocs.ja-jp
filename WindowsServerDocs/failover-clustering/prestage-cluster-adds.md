...
## <a name="step-3-grant-the-cno-permissions-to-the-ou-or-prestage-vcos-for-clustered-roles"></a>手順 3:CNO のアクセス許可を OU に付与するか、クラスター化された役割の Vco を事前設定します。
...
- オプション 1:[CNO のアクセス許可を OU に付与](#grant-the-cno-permissions-to-the-ou)します。 このオプションを使用すると、クラスターは自動的に AD DS 内に VCO を作成できます。 そのため、フェールオーバー クラスターの管理者は、AD DS 内で VCO がプレステージされていなくても、クラスター化された役割を作成できます。
...
1. [Active Directory ユーザーとコンピューター] の **[表示]** メニューで、 **[拡張機能]** がオンになっていることを確認します。
2. CNO を作成した OU を右クリックし [Step 1:AD DS @ no__t-0 で CNO を事前設定し、 **[プロパティ]** を選択します。
...
9. **[アクセス許可]** で **[コンピューター オブジェクトの作成]** チェック ボックスをオンにします。
   ...
   **Figure 3。CNO @ no__t に "コンピューターオブジェクトの作成" アクセス許可を付与する

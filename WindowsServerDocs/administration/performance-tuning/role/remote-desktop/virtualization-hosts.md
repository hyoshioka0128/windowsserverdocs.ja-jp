...
ms.locfileid: "72812273"
---
# <a name="performance-tuning-remote-desktop-virtualization-hosts"></a>リモートデスクトップ仮想化ホストのパフォーマンスチューニング

リモートデスクトップ仮想化ホスト (RD 仮想化ホスト) は、仮想デスクトップインフラストラクチャ (VDI) シナリオをサポートする役割サービスであり、複数のユーザーが windows Server を実行しているサーバーでホストされている仮想マシンで Windows ベースのアプリケーションを実行できます。Hyper-v。

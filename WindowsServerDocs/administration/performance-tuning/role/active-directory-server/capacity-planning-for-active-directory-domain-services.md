...
|RAM|Database Size<br />基本オペレーティングシステムの推奨事項<br />サード パーティ製のアプリケーション|
|ネットワーク|1 GB|
...
> [!NOTE]
> データベースからの読み取りに焦点を当てます。これは通常、要求が最も厳しいコンポーネントであるため、LogicalDisk ( *\<NTDS log\>* ) \ Avg Disk Sec/Write および LogicalDisk ( *\<ntds log\>* ) \ writes/sec) を使用して、ログファイルへの書き込みに同じロジックを適用できます。
>  
...
- 基になる記憶域の容量が超過している記憶域に必要な i/o の量を確認するには、次のようにします。
  >*必要な IOPS* = (LogicalDisk ( *\<ntds データベースドライブ\>* ) \ Avg disk sec/Read &divide; *\<ターゲット avg disk sec/read\>* ) &times; LogicalDisk ( *\<NTDS データベースドライブ\>* ) \ 読み取り/秒
...
|-|-|-|-|-|
|プロセッサ|プロセッサ情報 (_Total)\\% Processor ユーティリティ|60 分|40%|60%|
|RAM (Windows Server 2008 R2 以前)|Memory\ 使用可能な MB|< 100 MB|N/A|< 100 MB|
...
|ストレージ|LogicalDisk ( *\<NTDS データベースドライブ\>* ) \ Avg Disk Sec/Read<p>LogicalDisk ( *\<NTDS データベースドライブ\>* ) \ Avg Disk Sec/Write|60 分|10 ms|15 ms|
...
1. 修飾子を決定するには、次の式を使用します。
   >((*コアスコア値ごとのターゲットプラットフォーム*) &times; (*ベースラインプラットフォームのコアあたり mhz*) &divide; ((*コアスコアの値ごとの基準*) &times; (*ターゲットプラットフォームのコアあたり mhz*)  
...
|説明|ボトルネック分析|ディスク|Bus|アダプタ|PCI バス|
|-|-|-|-|-|-|
...
> *スピンドルあたりの最大 IOPS* &times; 2 スピンドル &times; [( *% 読み取り* +  *% 書き込み*) &divide; ( *% 読み取り*+ 2 &times; *% writes*)] =*合計 i/o 数*  

raid 1 セットでは、RAID 1 セットの多重度 (*N*) がストライピングされている場合、処理できる I/o の合計は、次のように raid 1 ごとに N &times; i/o になります。  

> *N* &times; {*スピンドルあたりの最大 IOPS* &times; 2 スピンドル &times; [( *% 読み取り* +  *% 書き込み*) &divide; ( *% 読み取り*+ 2 &times; *% writes*)]} =*合計 iops*
...
> *スピンドルあたりの IOPS* &times; *(スピンドル*– 1) &times; [ *(%* 読み取り +  *% 書き込み*) &divide; ( *% 読み取り*+ 4 &times; *% writes*)] =*合計 iops*

...
3.  コレクター コンピューターで、次のコマンドのいずれかを実行します。  
    ...
    -   Windows PowerShell プロンプトで: `Set-Item -Force WSMan:\localhost\Client\TrustedHosts "<target1>,<target2>,..."`、その後に`Set-Item -Force WSMan:\localhost\Client\AllowUnencrypted true`場所\<target1 > など、ターゲット コンピューターの IP アドレスや名。  
  
    -   または、コマンド プロンプトで: **winrm winrm/構成/クライアントを設定する @{TrustedHosts ="\<target1 >、\<target2 >,..."です。AllowUnencrypted ="true"}**  
...
4.  コレクター コンピューターに移動し、次のいずれかの Windows PowerShell コマンドを実行して、リモート接続をテストします。  
    ...
    ターゲット コンピューターが、コレクター コンピューターと同じドメイン内にある場合は、実行します。 `New-PSSession -Computer <target> | Remove-PSSession`  
...
5.  対象のコンピューターで、管理者特権の Windows PowerShell プロンプトを開き、次のコマンドを実行します。  
    ...
    ここで、対象のコンピューターの名前は、< target_name > \<ip > はコレクター コンピューターの IP アドレスです。 \<ポート > ポート番号は、コレクターが実行されます。 Key <a.b.c.d> は通信に必要な暗号化キーで、ドットで区切られた 4 つの英数字文字列で構成されます。 コレクター コンピューターでは、これと同じキーが使用されます。 キーを入力しない場合、システムによってランダムなキーが生成されます。このキーはコレクター コンピューターで必要になるので、メモしておきます。  
...
1.  管理者特権のコマンド プロンプトを起動し、次のコマンドを実行します。  
    ...
    **bcdedit/eventsettings net hostip:1.2.3.4: ポート 50000 key:a.b.c.d**  
...
**イベント トランスポート自体が有効になったので、実際にそのトランスポート経由の ETW イベントを送信するシステムを有効にする必要があります。**  
...
3.  次のコマンドのいずれかを実行します。  
    ...
    管理者特権の Windows PowerShell プロンプト。 `Enable-SbecBcd -ComputerName <target_name> -CollectorIP <ip> -CollectorPort <port> -Key <a.b.c.d> -BusParams <X.Y.Z>`  
...
-   終点アドレス =\<コレクターの IP アドレス >  
  
-   ポート =\<コレクターを使用して、指定されたポート番号 >  
...
次の 3 つの XML 構成ファイルが作成されに格納されているサービスを有効にすると**c:\ProgramData\Microsoft\BootEventCollector\Config**:  
...
行う必要がある決定の 1 つとして、ファイル サイズの制限を設定することがあります。 最適なファイル サイズの制限は、予想されるイベントの量や使用可能なディスク領域によって異なります。 古いデータを消去するという観点からは、小さいファイルの方が便利です。 ただし、64 KB のヘッダーのオーバーヘッドが備える各ファイルと結合された履歴を取得する多くのファイルの読み取りできない可能性があります便利です。 絶対最小ファイル サイズの上限は、256 KB です。 適度に実用的なファイル サイズの制限は 1 MB 以上である必要があり、10 MB がおそらく一般的な適切な値です。 多くのイベントが予想される場合は、制限値を高くすることが妥当です。  
...
3.  次の構成例をメモ帳のウィンドウにコピーします。  
    ...
    > [!NOTE]  
    > ルート ノードは\<コレクター >。 その属性は、構成ファイルの構文のバージョンとステータス ログ ファイルの名前を指定します。  
    >   
    > \<共通 > 要素が複数のターゲット ユーザー グループを使用して、複数のユーザーの一般的なアクセス許可を指定することができますように、一般的な構成要素を非常によく指定でグループ化します。  
    >   
    > \<Collectorport > 要素は、コレクターは受信データをリッスンする UDP ポート番号を定義します。 これは、Bcdedit のターゲット構成の手順で指定されたのと同じポートです。 コレクターは 1 つのポートのみをサポートし、すべてのターゲットが同じポートに接続する必要があります。  
    >   
    > \<フォワーダー > 要素では、対象のコンピュータから受信した ETW イベントの転送方法を指定します。 フォワーダーは 1 種類のみで、イベントを ETL ファイルに書き込みます。 パラメーターは、ファイル名のパターン、リング内の各ファイルのサイズ制限、各コンピューターのリングのサイズを指定します。 "toxml" 設定は、ETW イベントが受信されたときに、XML に変換することなく、バイナリ形式で書き込まれるように指定します。 イベントを XML に変換するかどうかの決定については、「XML イベントの変換」に関するセクションを参照してください。 ファイル名のパターンでは、{computer} にコンピューター名を、{#3} にリング内のファイルのインデックスを代入します。  
    >   
    > このファイルの例では、2 つのターゲット コンピューターがで定義されて、\<ターゲット > 要素。 それぞれの定義を使用して、IP アドレスを指定します\<ipv4 >、MAC アドレスを使用することもできますが、(たとえば、< mac 値 =「有効」\/> または < value="11-22-33-44-55-66 mac"\/>) または (SMBIOS GUID例では、< guid 値 ="{269076F9-4B77-46E1-B03B-CA5003775B88}"\/>)、ターゲット コンピューターを識別するためにします。 また、暗号化キー (対象のコンピューターで Bcdedit で指定または生成されたものと同じ) およびコンピューター名を確認してください。  
  
4.  別個の各ターゲット コンピューターの詳細を入力\<ターゲット > 要素の構成ファイルでし Newconfig.xml と閉じるメモ帳を保存します。  
...
4.  Events フラグを有効にし、コレクター コンピューターを設定するように Nano Server イメージの BCD 設定を更新して、診断イベントが適切なサーバーに送信されることを確認します。 コレクター コンピューターの IPv4 アドレス、TCP ポート、およびコレクターの Active.XML ファイルで構成した暗号化キー (このトピックの他の場所で説明されています) を書き留めます。 管理者特権を持つ Windows PowerShell コンソールで、このコマンドを使用します。 `Enable-SbecBcd -Path C:\NanoServer\Workloads\IncludingWorkloads.vhd -CollectorIp 192.168.100.1 -CollectorPort 50000 -Key a.b.c.d`  
...
最小のログをレベルに設定する、\<コレクター > 構成ファイルの要素。 例: < コレクター configVersionMajor =「1」minlog\="verbose">。  
  
詳細レベルでは、受信した各パケットが処理されるときにログに記録されます。 デバッグ レベルでは、処理の詳細がさらに追加され、受信されたすべての ETW パケットのコンテンツもダンプされます。  
  
デバッグ レベルでは、通常のログ システムで表示するのではなく、ファイルにログを書き込むと便利です。 これを行うには、その他の要素を追加、\<コレクター > 構成ファイルの要素。  
...
|コレクター||ETL ファイルが作成されていない。|`Get-SbecForwarding` ETL ファイルは作成されませんが、エラーなしで、ターゲットが接続を示しています。|対象のコンピューターでおそらくデータがまだ送信されていない可能性があります。ETL ファイルは、データが受信されたときにのみ作成されます。|  

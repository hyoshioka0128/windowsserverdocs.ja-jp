...
### <a name="-disable-syntax-and-parameters"></a><a name=BKMK_disableprot></a>構文とパラメーターを無効にします。
```
...
## <a name="examples"></a>例
**-プロテクター**コマンドを使用して、証明書ファイルによって識別される証明書キー保護機能を E ドライブに追加する方法を示します。
```
manage-bde  -protectors  -add E: -certificate  -cf c:\File Folder\Filename.cer
```
**-プロテクター**コマンドを使用して、ドメインとユーザー名で識別される**adaccountorgroup**キープロテクターをドライブ E に追加する方法を示します。
```
manage-bde  -protectors  -add E: -sid DOMAIN\user
```
コンピューターが3回再起動されるまで保護を無効にするために**プロテクター**コマンドを使用する方法を示します。
```
manage-bde  -protectors  -disable C: -rc 3
```
**-プロテクター**コマンドを使用して、C ドライブのすべての TPM とスタートアップキーに基づくキープロテクターを削除する方法を示します。
```
manage-bde  -protectors -delete C: -type tpmandstartupkey
```
**-プロテクター**コマンドを使用して、C ドライブのすべての回復情報を AD DS にバックアップする方法については、「」を参照してください。
```

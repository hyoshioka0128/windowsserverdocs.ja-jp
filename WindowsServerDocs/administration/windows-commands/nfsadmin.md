...
**audit\=** \[ **\+** | **\-** \]**マウント**\[ **\+|\-** \]**読み取り**\[ **\+|\-** **\]\[** \+ **|** \- **\]\[** \+| **\+** \-\] **\[** \+**delete**|\- **\+** \]\[ **\-** \+を**locking**|\- **\]** **\-** **write** **create** **\-** **all**  
...
-   ネットワーク ファイル システムに作成されるファイルの既定のアクセス許可モードを指定 \(NFS\) サーバーです。 *モード* 引数は、0 から 7 への 3 桁で構成されます \(包括的な\) ユーザー、グループ、およびその他のユーザーに与える既定のアクセス許可を表す \(それぞれ\)します。 数字は、次のように UNIX\-スタイルのアクセス許可に変換されます。0\=none、1\=x、2\=w、3\=wx、4\=r、5\=rx、6\=rw、7\=rwx。 たとえば、 **fileaccess\=750** 利用 rwx のアクセス許可の所有者に、rx アクセス許可、グループ、および他のユーザーへのアクセス権限がありません。  

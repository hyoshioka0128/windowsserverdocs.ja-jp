...
|/ru {[\<ドメイン >\]<User> | システム}|変更するタスクを実行するユーザー アカウントを指定します。 有効なエントリには、ローカル システム アカウントを指定するには、""、"NT authority \system"または「システム」です。</br>ユーザー アカウントを変更するとは、また、ユーザーのパスワードを変更する必要があります。 コマンドがある場合、 **/ru** パラメーターではなく、 **/rp** パラメーター、 **schtasks** 新しいパスワードの入力を要求します。</br>ローカル システム アカウントのアクセス許可でタスクを実行したりしないでください必要なパスワードを要求します。|
...
|用語|定義|
|----|----------|
|/tn {\<TaskName>| *}|必須。 削除するタスクを識別します。</br>**\<TaskName >** -名前付きのタスクを削除します。</br>**<\*>** -コンピューター上のすべてのスケジュールされたタスクを削除します。|
...
```
これに対して、 **SchTasks.exe** 次の確認メッセージが表示されます。 タスクを削除するには、Y キーを押します。**します。** コマンドを取り消す場合に、次のように入力します **n**:。
```
...
次のコマンドは、他のユーザーがスケジュールされたタスクを含めて、ローカル コンピューターのスケジュールからすべてのタスクを削除します。 使用して、 **/tn \*** をコンピューター上のすべてのタスクを表すパラメーターおよび **/f** を抑制する状況の確認メッセージのパラメーターです。
```
...
|/fo {テーブル| 一覧| CSV}|出力形式を指定します。 **テーブル** 既定値です。|

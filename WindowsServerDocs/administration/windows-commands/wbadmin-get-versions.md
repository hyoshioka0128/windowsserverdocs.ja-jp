...
description: Wbadmin get バージョンの Windows コマンドに関するトピック。ローカルコンピューターまたは別のコンピューターに格納されている利用可能なバックアップの詳細を一覧表示します。
...
```
リモート共有フォルダーに格納されている使用可能なバックアップの一覧を表示するには \\\\servername\share のコンピューター server01 で、次のように入力します。
```

...
-   ドライブをネットワークに接続します。  
    ネットワークにアクセスするスケジュールされたジョブは、リダイレクトされたドライブを使用しないでください。 スケジュール サービスで、リダイレクトされたドライブにアクセスできないか、リダイレクトされたドライブは、スケジュールされたタスクの実行時に、別のユーザーがログオンしている場合に存在できない可能性があります。 代わりに、スケジュールされたジョブの UNC パスを使用します。 次に、例を示します。  

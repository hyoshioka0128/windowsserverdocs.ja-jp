...
|/a [[:] \<Attributes >]|指定した属性を持つこれらのディレクトリとファイルの名前のみを表示します。 **/A**を省略した場合は、隠しファイルとシステムファイルを除くすべてのファイルの名前が**表示され**ます。 *属性*を指定せずに **/a**を使用すると、隠しファイルやシステムファイルなど、すべてのファイルの名前が**dir**に表示されます。</br>次の一覧では、*属性*に使用できる各値について説明します。 コロン (:) を使用するは省略可能です。 これらの値の任意の組み合わせを使用し、値をスペースで区切ることは避けてください。</br>**d**ディレクトリ</br>**h** ファイルを非表示</br>**s** システム ファイル</br>**l** 再解析ポイント</br>**r** 読み取り専用ファイル</br>**a**アーカイブ ファイル</br>**i**コンテンツ インデックス付きのファイルがありません</br>**-** プレフィックスが "not" であることを意味します。|
|/o [[:] \<SortOrder >]|並べ替え*順序*に従って出力を並べ替えます。次の値の任意の組み合わせを指定できます。</br>名前で**n** (アルファベット順)</br>拡張子による**e** (アルファベット順)</br>最初にディレクトリ**をグループ化**する</br>**s** (サイズ順) (最小値)</br>日付/**時刻別 (最も古い順)**</br>**-** 順序を逆にするプレフィックス</br>メモ:コロンの使用は省略可能です。 複数の値は、一覧の順序に従って処理されます。 複数の値をスペースで区切ることは避けてください。</br>*順序*付けが指定されていない場合、ディレクトリはアルファベット順**に一覧表示**され、ファイルがアルファベット順に並べ替えられます。|

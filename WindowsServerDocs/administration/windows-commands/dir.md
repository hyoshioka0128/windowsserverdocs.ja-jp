...
|/a [[:]\<属性 >]|指定した属性を持つこれらのディレクトリとファイルの名前のみを表示します。 **/A**を省略した場合は、隠しファイルとシステムファイルを除くすべてのファイルの名前が**表示され**ます。 *属性*を指定せずに **/a**を使用すると、隠しファイルやシステムファイルなど、すべてのファイルの名前が**dir**に表示されます。</br>次の一覧では、*属性*に使用できる各値について説明します。 コロン (:) を使用するは省略可能です。 これらの値の任意の組み合わせを使用し、値をスペースで区切ることは避けてください。</br>**d**ディレクトリ</br>**h**隠しファイル</br>**s**システムファイル</br>**左**再解析ポイント</br>**r**読み取り専用ファイル</br>アーカイブの準備**が**できているファイル</br>コンテンツにインデックスが**設定され**ていないファイル</br>**-** プレフィックスが "not" であることを意味します。|

---
title: wscript
description: Wscript の Windows コマンドに関するトピック。これは、さまざまなオブジェクトモデルを使用してタスクを実行するさまざまな言語でスクリプトをユーザーが実行できる環境を提供します。
...
|/t:\<数 >|スクリプトを実行できる最長時間を秒単位で指定します。 最大32767秒を指定できます。</br>既定では、時間制限はありません。|
|/x|デバッガーでスクリプトを開始します。|

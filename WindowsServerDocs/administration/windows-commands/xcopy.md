---
title: xcopy
description: Xcopy の Windows コマンドに関するトピックでは、ファイルとディレクトリ (サブディレクトリを含む) をコピーします。
...
**6.** \ Customer ディレクトリとすべてのサブディレクトリを \\\\のディレクトリにコピーするには、次のように、読み取り専用の属性を保持し、h: のように新しいファイルが作成されたときに、次のように入力します。
...
前の例では、この特定の source パラメーターの値を **\*\\します。 .yml**は、2つのパス **\\** 文字が削除された場合でも、同じ3つのファイルをコピーします。 ただし、source パラメーターからアスタリスクのワイルドカードが削除された場合、ファイルはコピーされません **。 .yml を\\** 。

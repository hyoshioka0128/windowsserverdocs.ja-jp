...
前の例では、この特定のソースパラメーター値を使用**しています。 \\\*.yml** 2 つの**パス文字 \\ が**削除された場合でも、同じ3つのファイルをコピーします。 ただし、source パラメーターからアスタリスクのワイルドカードが削除された場合、ファイルはコピーされません **。 \\.yml**。

...
|-上書き|ファイルを回復する場合にのみ有効です。 回復中のファイルが同じ場所に既に存在する場合に実行する操作を指定します。</br>-   **Skip**を使用すると、既存のファイルをスキップし、次のファイルの回復を続行でき Windows Server バックアップます。</br>-    **"createcopy"** の場合、既存のファイルが変更されないように、既存のファイルのコピーが Windows Server バックアップによって作成されます。</br>-   **Overwrite**の場合、Windows Server バックアップによって、バックアップからのファイルで既存のファイルが上書きされます。|
...
```
2013年3月31日から9:00 午前0時に実行されたバックアップの回復を実行するには \\ @ no__t? \Volume{cc566d14-44a0-11d9-9d93-806e6f6e6963} @no__t 種類:
```
wbadmin start recovery -version:03/31/2013-09:00 -itemType:Volume 
-items:\\?\Volume{cc566d14-44a0-11d9-9d93-806e6f6e6963}\
```
2013年4月30日の午前9:00 に実行されたバックアップの回復を実行するには、共有フォルダー \\ @ no__t-1servername\share @ server01 に次のように入力します。
```

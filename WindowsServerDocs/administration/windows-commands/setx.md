---
title: setx
description: Windows コマンドに関する setx のトピックでは、ユーザーまたはシステム環境で環境変数を作成または変更します。プログラミングやスクリプトは必要ありません。
...
|      /d \<区切り記号 >      |                    などの区切り記号を指定します。また **、** 4 つの組み込みの区切り記号 (スペース、タブ、ENTER、およびラインフィード) に加えて使用する **\\** を指定します。 有効な区切り記号には、ASCII 文字が含まれます。 区切り文字の最大数は、15、組み込みの区切り記号を含みます。                    |
...
```
ローカル環境の OCTET1 環境変数を、座標5で見つかった値に設定するには、ファイルを区切り記号と共に使用**して \*#$します。**
```

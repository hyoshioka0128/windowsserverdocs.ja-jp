...
|-ClientCertificate *\<ClientCertId >*|*\<ClientCertID >* を、証明書の拇印、CN、EKU、template、EMAIL、UPN、および新しい name = value 構文に置き換えることができます。|
|-ユーザー名 *\<ユーザー名 >*|証明書の登録 Web サービスで使用されます。 *\<UserName >* を SAM 名または domain\user に置き換えることができます。 このオプションは、-p オプションと共に使用します。|

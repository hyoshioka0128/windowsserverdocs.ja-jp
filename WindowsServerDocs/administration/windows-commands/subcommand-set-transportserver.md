...
description: Windows コマンドに関するトピックでは、トランスポートサーバーの構成設定を設定するサブコマンド set TransportServer について説明します。

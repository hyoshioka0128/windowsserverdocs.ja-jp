---
title: dfsrmig
description: Dfsrmig のリファレンストピックでは、ファイルレプリケーションサービス (FRS) から分散ファイルシステム (DFS) レプリケーションに SYSvol レプリケーションを移行し、移行の進行状況に関する情報を提供して、移行をサポートするために active directory ドメインサービス (AD DS) オブジェクトを変更します。

---
title: WSUS 展開を計画します。
description: Windows Server Update Service (WSUS) のトピック-関連トピックへのリンクを含む展開計画プロセスの概要
...
|タスク|説明|
|----|--------|
|[1.1。考慮事項とシステム要件を確認する @ no__t-0|考慮事項とシステム要件の一覧を確認して、WSUS の展開に必要なすべてのハードウェアとソフトウェアが揃っていることを確認します。|
|[1.2。WSUS の展開シナリオを選択する @ no__t-0|使用する WSUS の展開シナリオを決定します。|
|[1.3。WSUS 記憶域の戦略を選択する @ no__t-0|展開に最適な WSUS 記憶域の戦略を決定します。|
|[1.4。WSUS 更新プログラムの言語を選択する @ no__t-0|インストールする WSUS 更新プログラムの言語を決定します。|
|[1.5。WSUS コンピューターグループを計画する @ no__t-0|展開に使用する WSUS コンピューター グループのアプローチを計画します。|
|[1.6。WSUS のパフォーマンスに関する考慮事項を計画する:バックグラウンドインテリジェント転送サービス @ no__t-0|パフォーマンスが最適になるように WSUS の設計を計画します。|
|[1.7。自動更新設定を計画する @ no__t-0|シナリオの自動更新の設定をどのように構成するかを計画します。|
...
-   WSUS の役割を有効にする際に求められるサーバーのハードウェア要件は、ハードウェア要件にバインドされています。 WSUS の最小ハードウェア要件
    ...
    -   **Sp**1.4 ギガヘルツ (GHz) の x64 プロセッサ (2 Ghz 以上を推奨)

    -   **量**WSUS では、サーバーおよび他のすべてのサービスまたはソフトウェアで必要とされる容量よりも 2 GB の RAM が必要です。

    -   **使用可能なディスク領域:** 10 GB (40 GB 以上を推奨)

    -   **ネットワークアダプター:** 100メガビット/秒 (Mbps) 以上

-   ソフトウェアの要件:

    -   レポートを表示するため、WSUS には [Microsoft Report Viewer Redistributable 2008](https://www.microsoft.com/download/details.aspx?id=6576)が必要です。 Windows Server 2016 では、WSUS には[Microsoft Report Viewer ランタイム 2012](https://www.microsoft.com/download/details.aspx?id=35747)が必要です
...
1. オンデマンド機能の使用によりペイロードが削除されたサーバーの役割を、Microsoft Update でオンデマンドでインストールすることはできません。 このようなサーバーの役割をインストールしようとしたときにインストールソースを提供するか、グループポリシーでオンデマンド機能のソースを構成する必要があります。
...
3. Windows 10 バージョン1709またはバージョン1803を実行しているエンタープライズデバイスでは、WSUS から直接オンデマンドで機能をインストールすることはできません。 必要に応じて機能をインストールするには、[機能ファイル (サイドバイサイドストア) を作成](https://docs.microsoft.com/previous-versions/windows/it-pro/windows-server-2012-R2-and-2012/jj127275%28v=ws.11%29#create-a-feature-file-or-side-by-side-store)するか、次のいずれかのソースからオンデマンドの機能パッケージを取得します。
   - [ボリュームライセンスサービスセンター](https://www.microsoft.com/licensing/servicecenter) (VLSC)-VL アクセスが必要です
   - OEM ポータル-OEM アクセスが必要です
   - MSDN ダウンロード-MSDN サブスクリプションが必要です

     個別に取得された機能のオンデマンドパッケージは、 [DISM コマンドラインオプション](https://docs.microsoft.com/windows-hardware/manufacture/desktop/dism-operating-system-package-servicing-command-line-options)を使用してインストールできます。
...
3.  データベースサーバーは、WSUS サーバーと同じ active directory ドメインに存在するか、WSUS サーバーの active directory ドメインと信頼関係にある必要があります。

4.  WSUS サーバーとデータベースサーバーは、同じタイムゾーンに存在するか、同じ協定世界時 (グリニッジ標準時) ソースに同期されている必要があります。
...
管理者は、組織のイントラネット内のすべてのコンテンツを同期する WSUS を実行する複数のサーバーを展開できます。 1台のサーバーのみをインターネットに公開することがあります。これは、Microsoft Update から更新プログラムをダウンロードする唯一のサーバーです。 このサーバーがセットアップされて、アップ ストリーム サーバーとダウン ストリーム サーバーが同期ソースです。 すべてのクライアント コンピューターに最適な接続を提供するために、必要に応じて、サーバーを地理的に離れたネットワーク全体に配置することができます。
...
> [!NOTE] 
> 作成する WSUS サーバーの階層は 3 階層までにすることをお勧めします。 各レベルでは、接続されたサーバー全体に更新プログラムを伝達するのに時間がかかります。 階層には理論的な制限はありませんが、Microsoft によってテストされているのは、階層が5レベルの展開のみです。
>
> また、ダウンストリームサーバーは、アップストリームサーバーの同期ソースと同じバージョンまたは以前のバージョンの WSUS である必要があります。
...
1.  WAN リンクの使用率を減らし、アプリケーションの応答性を向上させることができます。 WSUS サーバーで扱われるコンテンツについて、BranchCache による高速化を有効にするには、サーバーとクライアントに BranchCache 機能をインストールし、BranchCache サービスが開始されたことを確認します。 それ以外の手順は必要ありません。
...
ネットワークに、さまざまな場所からネットワークにログオンするモバイル ユーザーが含まれている場合、移動ユーザーが地理的に最も近い WSUS サーバーからクライアント コンピューターを更新できるように WSUS を構成できます。 たとえば、リージョンごとに1つの WSUS サーバーを展開し、リージョンごとに異なる DNS サブネットを使用することができます。 すべてのクライアントコンピュータを同じ WSUS サーバーに転送できます。このサーバーは、各サブネットで最も近い物理 WSUS サーバーに解決されます。
...
Windows Internal Database にはユーザー インターフェイスもデータベース管理ツールも用意されていません。 WSUS にこのデータベースを選択する場合、外部ツールを使用してデータベースを管理する必要があります。 詳細については、以下をご覧ください。

-   [WSUS データのバックアップと復元およびサーバーのバックアップ](https://technet.microsoft.com/library/dd939904(WS.10).aspx)

-   [WSUS データベースの再インデックス](https://technet.microsoft.com/library/dd939795(WS.10).aspx)
...
このオプションを使用するには、サーバーに、必要な更新プログラムをすべて保存するための十分な空きディスク領域が必要です。 WSUS で更新プログラムをローカルに保存するには、少なくとも 20 GB が必要です。ただし、テスト済みの変数に基づいて 30 GB をお勧めします。
...
WSUS 構成ウィザードの **[言語の選択]** ページを使用すると、すべての言語または一部の言語から更新プログラムを取得できます。 一部の言語を選択すると、ディスク領域が節約されますが、WSUS サーバーのすべてのダウンストリームサーバーとクライアントコンピューターで必要なすべての言語を選択することが重要です。

このオプションを構成する前に覚えておく必要がある、更新プログラムの言語に関する重要な注意事項を次に示します。
...
更新プログラムは、その言語を必要とするクライアント コンピューターで **[適用できません]** と表示されます。 この問題を回避するには、すべてのオペレーティング システムの言語が WSUS サーバーの同期のオプションに含まれていることを確認してください。 WSUS 管理コンソールの **[コンピューター]** ビューに移動し、オペレーティングシステムの言語によってコンピューターを並べ替えることにより、すべてのオペレーティングシステムの言語を確認できます。 ただし、(Microsoft Word のフランス語版は、Windows 8 の英語バージョンを使用する一部のコンピューターでインストールされる場合など、複数の言語でマイクロソフトのアプリケーションがある場合より多くの言語を含む必要な場合があります。
...
コンピューターは複数のグループに割り当てることができるため、同じコンピューターで 1 つの更新プログラムを何度も承認することができます。 ただし、その更新プログラムが展開されるのは一度だけです。競合が発生した場合は WSUS サーバーで解決されます。 前の例を続行するには、[更新プログラム 1] が [Payroll] グループに割り当てられていて、[買掛金] グループに [アカウント] が承認されている場合は、1回だけデプロイされます。
...
更新サービスコンソールの 階層 ウィンドウの次のテキスト例では、WSUS-01 という名前の WSUS サーバーに対して、デスクトップコンピューターとサーバー という名前のコンピューターグループが既定の **すべてのコンピューター** グループに追加されています。 デスクトップコンピューターとサーバーグループの両方が同じ階層レベルにあります。
...
この例では、Desktop computers 分岐 (デスクトップ L2) の下の2つのレベルは、サーバーブランチ (Servers L1) の1レベル下のグループより高い優先順位を持ちます。 したがって、Desktops-L2 グループと Servers-L1 グループの両方のメンバーであるコンピューターの場合、Desktops-L2 グループに対するすべての処理の方が、Servers-L1 グループに対して指定されている処理よりも優先されます。
...
WSUS を展開する前に、パフォーマンスを最適化できるように、慎重に計画する必要がある領域がいくつかあります。 主要な分野は次のとおりです。
...
1.  バックグラウンド インテリジェント転送サービス (BITS) の調整を使用します。 BITS の帯域幅の制限は時刻で管理できますが、BITS を使用しているすべてのアプリケーションに適用されます。 BITS の調整を制御する方法については、「[グループポリシー](https://msdn.microsoft.com/library/windows/desktop/aa362844(v=vs.85).aspx)」を参照してください。
...
WSUS はすべてのファイル転送タスクにバックグラウンド インテリジェント転送サービス (BITS) プロトコルを使用します。 これには、クライアント コンピューターへのダウンロードと、サーバーの同期が含まれます。 BITS では、プログラムが予備の帯域幅を使用してファイルをダウンロードできます。 BITS は、ネットワークの切断やコンピューターの再起動が発生しても、ファイルの転送を維持します。 詳細については、以下をご覧ください。[バックグラウンドインテリジェント転送サービス](https://msdn.microsoft.com/library/bb968799.aspx)。

...
SPA は、SPA フレームワークのすべてのバイナリ、高度なシナリオで使用される Windows PowerShell コマンドレット、およびコア OS、Hyper-v、active directory、および IIS の各 advisor パックを含む .cab ファイルとしてパッケージ化されています。 フォルダーに .cab ファイルを抽出した後で追加のインストールは必要ありません。 ただし、SPA を実行するには、次のように、対象サーバーからのデータ収集を有効にする必要があります。
...
3.  **[次へ]** をクリックします。 **[プロジェクトデータベースの作成]** ページでは、データベースを作成する Microsoft SQL Server インスタンスの名前を指定するよう求められます。 たとえば、コンソールと同じコンピューター上にある場合は、 **SQL server 名&gt;&lt;localhost\\** を使用できます。
...
* セキュリティの設定 **ネットワーク アクセス: ネットワーク認証のための資格情報およびパスワードの保存を許可しない** は無効になります。 SPA は、ユーザーの資格情報を使用して、ターゲット サーバー上のデータ コレクション セットを作成する必要があるために、セキュリティの設定を無効にする必要があります。

...
|Memory|ジェネレーション 2; 12 TB <br>第 1 世代の場合、1 TB|特定のオペレーティング システムの要件を確認して、最大値および推奨値を決定してください。|  
|シリアル (COM) ポート|2|[なし] :|  
...
|ストレージ|による制限は、ホスト オペレーティング システムでサポートされています。 Hyper-V による制限はありません。|**注:** Microsoft は、SMB 3.0 を使用する場合に、ネットワーク接続ストレージ (NAS) をサポートしています。 NFS ベースのストレージはサポートされません。|
...
仮想マシンの新機能を含むフェールオーバー クラスタリングの更新については、「[Windows Server 2016 のフェールオーバー クラスタリングの新機能](../../../failover-clustering/whats-new-in-failover-clustering.md)」を参照してください。

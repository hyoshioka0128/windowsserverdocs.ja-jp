...
3. オプションで **[スロットル率を指定する]** を使用してトラフィックのスロットル調整を有効にし、スロットル率を設定できます。 スロットル率には1より大きい値を指定する必要\(があります。\)また、1秒あたりのキロバイト\)数 (kb) またはメガバイト/秒\((MBps) を指定できます。
...
- **この実行可能ファイル名を持つアプリケーションのみ**が、QoS ポリシーウィザードの最初のページのトラフィック管理設定が特定のアプリケーション用であることを指定します。 実行可能ファイルの名前には、末尾にファイル名拡張子 .exe が付いている必要があります。
...
URL は、 [RFC 1738](https://tools.ietf.org/html/rfc1738)の`http[s]://<hostname\>:<port\>/<url-path>`形式でに準拠している必要があります。 `‘*'`ワイルドカードを使用することもでき`<hostname>`ます。 `https://training.\*/, https://\*.\*`場合`<port>`によっては、やのようにします。 `<hostname>`ただし`<port>`、ワイルドカードはまたはの部分文字列を表すことはできません。

つまり、と`https://\*training\*/`はどちら`https://my\*site/`も有効ではありません。 

必要に応じて、 **[サブディレクトリとファイルを含める]** をオンにして、URL の後にあるすべてのサブディレクトリとファイルに対して照合を実行できます。 たとえば、このオプションがオンになっていて、 `https://training`URL がの場合、QoS ポリシー` https://training/video`は適切な一致の要求を検討します。
...
2.  **[Qos ポリシー]** を右クリックし、 **[Qos の詳細設定]** をクリックします。
     ...
     次の図は、2つの [QoS の詳細設定] タブを示しています。**受信 TCP トラフィック**と**DSCP マーキングオーバーライド**。
...
これらの DSCP 値を使用する QoS ポリシーを作成して、WMM ワイヤレスアダプター\-の wi-fi 認定™を持つポータブルコンピューターが、WMM アクセスポイントに\-対して wi-fi 認定に関連付けられている場合に優先順位付きの処理を受けるようにすることができます。
...
ネットワーク5つ内のポリシーの競合では、最も一致する条件を持つポリシーが優先されます。 たとえば、policy_C は、発信元 IP アドレス "any"、宛先 IP アドレス10.0.0.1、発信元ポート "any"、宛先ポート "any"、およびプロトコル "TCP" を指定しているとします。 

次に、policy_D は発信元 IP アドレス "any"、宛先 IP アドレス10.0.0.1、発信元ポート "any"、宛先ポート80、およびプロトコル "TCP" を指定していると仮定します。 次に、policy_C と policy_D は両方とも宛先 10.0.0.1:80 への接続と一致します。 QoS ポリシーは、最も具体的な一致条件を持つポリシーを適用するため、この例では policy_D が優先されます。  

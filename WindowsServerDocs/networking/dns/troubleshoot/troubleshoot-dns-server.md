...
1. コマンドプロンプトでを実行 `ipconfig /all` し、IP アドレス、サブネットマスク、およびデフォルトゲートウェイを確認します。
...
3. セカンダリサーバーで、DNS コンソール内から、または次のコマンドを実行して、ゾーンを強制的に転送します。
   ...
   たとえば、ゾーンが corp.contoso.com の場合は、「」と入力し `dnscmd /zonerefresh corp.contoso.com` ます。
...
このサーバーがクエリを別のサーバーに転送しない場合は、このサーバーがルートサーバーを照会できるかどうかをテストします。 そのためには、次のコマンドを実行します。

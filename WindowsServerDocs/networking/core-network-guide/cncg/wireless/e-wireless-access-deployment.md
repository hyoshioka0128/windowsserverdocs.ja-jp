...
19. 認証の第 1 フェーズでユーザー id をマスクすることを指定するには、選択**Id プライバシーを有効にする**、およびテキスト ボックスに、テキスト ボックスを空白のままにまたは、匿名 id の名前を入力します。
    ...
    >- EAP id プライバシーが、空、または匿名の id に特定の EAP メソッドによって提供される\(実際の id と異なる\)は、EAP id 要求への応答で送信されます。 PEAP では、認証中に 2 回、id を送信します。 最初のフェーズでは、id はプレーン テキストで送信し、この id はクライアント認証ではなく、ルーティングの目的で使用します。 実際の id、認証に使用される — は最初のフェーズで規定されているセキュリティで保護されたトンネル内で、認証の 2 番目のフェーズ中に送信します。 場合**Id プライバシーを有効にする**チェック ボックスが選択されている場合、ユーザー名は、テキスト ボックスで指定されたエントリに置き換えられます。 たとえば、 **Id プライバシーを有効にする**が選択されていると id のプライバシーに関する別名**匿名**は、テキスト ボックスで指定します。 実際の id の別名を持つユーザーの**jdoe@example.com**、認証の最初のフェーズで送信された id に変更されます **anonymous@example.com**します。 ルーティングの目的で使用されているために、第 1 フェーズ id の領域の部分は変更されません。  

...
## <a name="BKMK_overview"></a>コアネットワークの概要
...
## <a name="BKMK_planning"></a>コアネットワークの計画
...
### <a name="bkmk_NetFndtn_Pln_Subnt"></a>サブネットの計画
...
### <a name="bkmk_NetFndtn_Pln_AllSrvrs"></a>すべてのサーバーの基本的な構成を計画する
...
ネットワーク全体の整合性を保つため、サーバー、プリンター、およびその他のデバイスに一貫した名前を付けることが推奨されます。 コンピューター名によって、ユーザーと管理者は、サーバー、プリンターなどのデバイスの用途と場所を識別しやすくなります。 たとえば、3つの DNS サーバー (サンフランシスコに1つ、ロサンゼルスに1つ、シカゴに1つ) がある場合は、名前付け規則の*サーバー関数*- の*場所*@no__t 3 を使用することができ*ます。*
...
|Active Directory Domain Services データベース フォルダーの場所|E:\Configuration @ no__t-0<br /><br />または既定の場所を受け入れる。|
|Active Directory Domain Services ログ ファイル フォルダーの場所|E:\Configuration @ no__t-0<br /><br />または既定の場所を受け入れる。|
|Active Directory Domain Services SYSVOL フォルダーの場所|E:\Configuration @ no__t-0<br /><br />または既定の場所を受け入れる。|
|ディレクトリ復元モード Administrator パスワード|**J @ no__t-1p2leO4 $ F**|
...
In-addr.arpa ドメインは、インターネットプロトコルバージョン 4 (IPv4) アドレス指定に基づくすべての TCP/IP ネットワークに適用されます。 新しいゾーン ウィザードでは、新しい逆引き参照ゾーンを作成するときに、このドメインが使用されているものと自動的に想定します。
...
### <a name="bkmk_NetFndtn_Pln_DomAccess"></a>ドメインアクセスの計画
...
## <a name="BKMK_deployment"></a>コアネットワークの展開
...
### <a name="BKMK_configuringAll"></a>すべてのサーバーの構成
...
#### <a name="BKMK_rename"></a>コンピューターの名前を変更する
...
#### <a name="BKMK_ip"></a>静的 IP アドレスを構成する
...
#### <a name="BKMK_deployADDNS01"></a>DC1 の展開
...
#### <a name="BKMK_installAD-DNS"></a>新しいフォレストの AD DS と DNS をインストールする
...
##### <a name="BKMK_createUA"></a>Active Directory ユーザーとコンピューターにユーザーアカウントを作成する
...
##### <a name="BKMK_assigngroup"></a>グループメンバーシップの割り当て
...
2.  詳細ウィンドウで、メンバーを追加するグループが含まれているフォルダーをダブルクリックします。
    ...
    -   **Active Directory ユーザーとコンピューター**/*ドメインノード*/*フォルダーが含まれているフォルダー*
...
##### <a name="BKMK_reverse"></a>DNS 逆引き参照ゾーンを構成する
...
3.  **[逆引き参照ゾーン]** をクリックし、 **[逆引き参照ゾーン]** を右クリックして、 **[新しいゾーン]** をクリックします。 新しいゾーンウィザードが開きます。
...
#### <a name="BKMK_joinlogserver"></a>ドメインへのサーバーコンピューターの参加とログオン
...
#### <a name="BKMK_deployDHCP01"></a>DHCP1 のデプロイ
...
##### <a name="BKMK_installDHCP"></a>動的ホスト構成プロトコル (DHCP) をインストールする
...
##### <a name="BKMK_newscopeDHCP"></a>新しい DHCP スコープを作成してアクティブ化する
...
### <a name="BKMK_joinlogclients"></a>クライアントコンピューターのドメインへの参加とログオン
...
### <a name="BKMK_optionalfeatures"></a>ネットワークアクセス認証と Web サービスのオプション機能の展開
...
#### <a name="BKMK_deployNPS1"></a>NPS1 のデプロイ
...
ネットワークポリシーサーバー (NPS) を使用すると、次の機能を使用してネットワークポリシーを一元的に構成および管理できます。リモート認証ダイヤルインユーザーサービス (RADIUS) サーバーと RADIUS プロキシ。
...
##### <a name="BKMK_installNPS"></a>ネットワークポリシーサーバー (NPS) をインストールする
...
##### <a name="BKMK_registerNPS"></a>NPS を既定のドメインに登録する
...
##### <a name="BKMK_install_IIS"></a>Web サーバー (IIS) サーバーの役割をインストールする
...
## <a name="BKMK_resources"></a>その他のテクニカルリソース
...
-   [Active Directory Domain Services の概要](https://technet.microsoft.com/library/hh831484.aspx)については https://technet.microsoft.com/library/hh831484.aspx をご覧ください。

-   @No__t-1 の[ドメインネームシステム (DNS) の概要](https://technet.microsoft.com/library/hh831667.aspx)。

-   [DNS 管理者ロールの実装](https://technet.microsoft.com/library/cc756152(WS.10).aspx)

-   @No__t-1 の[動的ホスト構成プロトコル (DHCP) の概要](https://technet.microsoft.com/library/hh831825.aspx)。

-   @No__t-1 での[ネットワークポリシーとアクセスサービスの概要](https://technet.microsoft.com/library/hh831683.aspx)について説明します。

-   @No__t-1 での[Web サーバー (IIS) の概要](https://technet.microsoft.com/library/hh831725.aspx)。

## <a name="BKMK_appendix"></a>付録 A ~ E
...
## <a name="BKMK_A"></a>付録 A-コンピューター名の変更
...
## <a name="BKMK_B"></a>付録 B-静的 IP アドレスの構成
...
## <a name="BKMK_C"></a>付録 C-コンピューターのドメインへの参加
...
## <a name="BKMK_D"></a>付録 D-ドメインにログオンする
...
## <a name="BKMK_E"></a>付録 E-コアネットワークの計画準備シート
...
### <a name="BKMK_FndtnPrep_InstallAD"></a>Active Directory Domain Services と DNS のインストール
...
#### <a name="BKMK_FndtnPrep_DNSRevrsLook"></a>DNS 逆引き参照ゾーンの構成
...
### <a name="BKMK_FndtnPrep_InstallDHCP"></a>DHCP のインストール
...
#### <a name="BKMK_FndtnPrep_DHCP_Exclusn"></a>DHCP での除外範囲の作成
...
#### <a name="bkmk_NetFndtn_Pln_DHCP_NewScope"></a>新しい DHCP スコープを作成しています

...
-   [Windows タイムサービスのアーキテクチャ](#windows-time-service-architecture)  
  
-   [Windows タイムサービス時間プロトコル](#windows-time-service-time-protocols)  
  
-   [Windows タイムサービスのプロセスと相互作用](#windows-time-service-processes-and-interactions)  
  
-   [Windows タイムサービスによって使用されるネットワークポート](#network-ports-used-by-windows-time-service)  
  
> [!NOTE]  
> このトピックでは、Windows タイム サービス (W32Time) の動作方法のみを説明します。 Windows タイムサービスを構成する方法の詳細については、「[高精度のシステムの構成](configuring-systems-for-high-accuracy.md)」を参照してください。
...
時刻が頻繁に同期されていない、またはドメインに参加していないコンピューターは、既定では time.windows.com と同期するように構成されています。  このため、断続的またはネットワーク接続がないコンピューターで時間の精度を保証することはできません。  
...
Windows タイム サービスを実行しているすべてのコンピューターでは、最も正確な時間を維持するために、サービスを使用します。 ドメインのメンバーであるコンピューターは、既定ではタイムクライアントとして機能します。そのため、ほとんどの場合、Windows タイムサービスを構成する必要はありません。 ただし、Windows タイムサービスは、指定された参照タイムソースから時間を要求するように構成できます。また、クライアントに時間を提供することもできます。

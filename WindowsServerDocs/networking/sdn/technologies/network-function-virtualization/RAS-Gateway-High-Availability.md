...
## <a name="bkmk_pools"></a>ゲートウェイプールの概要  
...
## <a name="bkmk_deployment"></a>RAS ゲートウェイの展開の概要  
...
## <a name="bkmk_integration"></a>RAS ゲートウェイとネットワークコントローラーの統合  
...
### <a name="bkmk_provisioning"></a>ゲートウェイ接続 (IKEv2、L3、GRE) のプロビジョニングと負荷分散  
...
障害検出、接続とルーティングの構成のスタンバイゲートウェイへのコピー、影響を受けた接続の BGP/静的ルーティングのフェールオーバー (引き出しなど) を含む、ネットワークコントローラーによる RAS ゲートウェイフェールオーバー応答のプロセスコンピューティングホストでのルートの再構成と、コンピューティングホストでの Hyper-v ネットワーク仮想化ポリシーの再構成は、GRE ゲートウェイと接続の場合と同じです。 ただし、GRE 接続の再確立は異なる動作をします。また、GRE の高可用性ソリューションには追加の要件があります。  
...
### <a name="bkmk_l3"></a>L3 転送ゲートウェイの高可用性  
...
|---------------|-------------------------|-----------|-------------------|  
|製薬|10.127.134.50|1001|10.127.134.55|  

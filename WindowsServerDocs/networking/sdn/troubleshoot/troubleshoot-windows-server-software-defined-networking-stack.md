...
* **無効またはサポートされない構成**ユーザーが NorthBound API を誤って呼び出すか、無効なポリシーを使用して呼び出されています。

* **ポリシーアプリケーションでエラーが発生**したネットワークコントローラーからのポリシーが Hyper-v ホストに配信されませんでした。すべての Hyper-v ホスト (たとえば、ライブマイグレーションの後) が大幅に遅れているか、最新の状態ではありません。
* **構成のずれまたはソフトウェアのバグ**データパスの問題が発生したため、パケットが破棄されました。

* **NIC ハードウェア/ドライバーまたはアンダーレイネットワークファブリックに関連する外部エラー**タスクのオフロード (VMQ など) またはアンダーレイネットワークファブリックの不適切な構成 (MTU など)
...
| Hyper-V ホスト | PA IP アドレス1 | PA IP アドレス2 |
|--|--|--|
...
1. [テナント]仮想サブネットまたはトラフィックをブロックする VM ネットワーク インターフェイスで指定された分散型のファイアウォール ポリシーがないことを確認します。

Sa18.nttest.microsoft.com ドメインの sa18n30nc にあるデモ環境で REST API ネットワークコントローラーに対してクエリを実行します。
...
これらのログには、ネットワーク コント ローラー クラスター、ネットワーク コント ローラー アプリケーション、ゲートウェイのログ、SLB、仮想ネットワークおよび分散型のファイアウォールのデバッグ ログが含まれます。 ネットワーク コント ローラーに新しいホスト/SLB/ゲートウェイを追加するたびにそれらのコンピューターのログ記録が開始されます。
同様に、ネットワーク コント ローラーからホスト/SLB/ゲートウェイが削除されると、それらのコンピューターのログが停止されました。
...
2.  検証、 *SDNSLBMPerfCounters* ネットワーク コント ローラーのノードの Vm (可能であれば、プライマリ ネットワーク コント ローラー ノード - Get NetworkControllerReplica) のいずれかのパフォーマンス モニターにします。
    ...
    4.  SLBM は Muxes に接続されているか。 (*Muxes 接続済み*  == SLBM で正常な状態*Muxes Healthy on SLBM*  == *Muxes レポート正常*= # SLB Muxes vm)。
...
3.  [ホスト]DIP エンドポイントの検出または接続されていない: 場合
    1.  確認 *デバッグ NetworkControllerConfigurationState*
        1.  NC および SLB ホストエージェントが、を使用してネットワークコントローラーのイベントコーディネーターに正常に接続されていることを検証します。``netstat -anp tcp |findstr 6640)``

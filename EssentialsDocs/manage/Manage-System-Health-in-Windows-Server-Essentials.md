...
> [!NOTE]
>  Windows Server Essentials と Windows Server Essentials エクスペリエンス役割がインストールされた Windows Server 2012 R2 で、サーバーとネットワーク内のクライアント コンピューターの正常性アラートはアラート ビューアーで表示されなくなりますが、の代わりに表示できます。**正常性レポート**のタブ、**ホーム**ページ。  
  
 Windows Server Essentials サーバーに接続されていて、マルウェア対策クライアントの古いウイルス定義が不足している管理者が問題に関連する重要な更新プログラムなど、システムの正常性アラートすべてのコンピューターを積極的に監視します。コンピューター、およびその他の重要な問題に対処が必要です。 これらの問題が、サーバーのダッシュ ボードまたは Windows Server essentials、または、クライアント コンピューターのスタート パッドから起動できる、アラート ビューアーにアラートとして表示されます、**正常性レポート**Windows Server Essentials でのタブ。 既定では、アラートは 30 分おきに更新されます。ただし、ネットワークのアラートはいつでも確認できます。アラート ビューアーまたは **[状態レポート]** タブの **[更新]** をクリックします。  
  
 次のトピックをご覧になれば、アラート ビューアーのアラートを確認し、対応する方法を理解できます。また、電子メールでアラート通知を受信するようにサーバーを設定する手順が記載されています。  
  
-   [正常性に関するレポートは、アドイン](Manage-System-Health-in-Windows-Server-Essentials.md#BKMK_AddIn)  
...
-   [アラートに応答するには](Manage-System-Health-in-Windows-Server-Essentials.md#BKMK_Respond)  
  
-   [アラートの電子メール通知を設定します。](Manage-System-Health-in-Windows-Server-Essentials.md#BKMK_Email)  
  
-   [潜在的なコンピューターのアラート](Manage-System-Health-in-Windows-Server-Essentials.md#BKMK_Potential)  
  
##  <a name="BKMK_AddIn"></a> 正常性に関するレポートは、アドイン  
...
-   [オンデマンドまたはスケジュールでレポートを生成します](Manage-System-Health-in-Windows-Server-Essentials.md#BKMK_Generate)  
  
-   [レポートの内容をカスタマイズします。](Manage-System-Health-in-Windows-Server-Essentials.md#BKMK_Customize)  
  
-   [電子メール レポート](Manage-System-Health-in-Windows-Server-Essentials.md#BKMK_emailreport)  
  
> [!NOTE]
>  **Windows Server Essentials:** 状態レポート アドインから Windows Server essentials をダウンロードすることができます、 [Microsoft ダウンロード センター](https://go.microsoft.com/fwlink/p/?LinkId=266342)します。  
>   
>  **Windows Server Essentials:** 既定では、正常性レポートを追加では Windows Server Essentials または Windows Server 2012 R2、Windows Server Essentials エクスペリエンス役割をインストールすると統合の正常性レポートが表示されます、**正常性レポート** タブダッシュ ボードの**ホーム**ページ。  
  
###  <a name="BKMK_Generate"></a> オンデマンドまたはスケジュールでレポートを生成します  
 ...
 レポートは手動で生成する以外に、毎日または毎時間のスケジュールで自動的に生成することもできます。 その場合、タスク ウィンドウで、 **[状態レポートの設定のカスタマイズ]** をクリックし、 **[スケジュールと電子メール]** タブをクリックします。 **[スケジュール]** 機能は既定でオフになっています。 **[スケジュールした時刻に状態レポートを生成する]** チェック ボックスを選択すると、オンになります。  
  
###  <a name="BKMK_Customize"></a> レポートの内容をカスタマイズします。  
 状態レポートには次が含まれます。  
...
- **記憶域** ドライブとその容量の一覧が **[詳細]** セクションに表示されます。  
  ...
  既定でレポートに含まれるデータ ポイントの一部に興味がない場合、レポートのコンテンツをカスタマイズできます。タスク ウィンドウの **[状態レポートの設定のカスタマイズ]** をクリックし、 **[コンテンツ]** タブをクリックします。レポートに表示するコンテンツのチェック ボックスをオフにします。 たとえば、独自のサーバー バックアップ計画があるあり、サーバー バックアップに関する警告を表示しない場合から除外できますサーバー バックアップ、レポートをオフにして、**サーバーのバックアップ**チェック ボックスをオンします。  
...
##  <a name="BKMK_Respond"></a> アラートに応答するには  
...
-   [警告を削除します。](Manage-System-Health-in-Windows-Server-Essentials.md#BKMK_4)  
...
##  <a name="BKMK_Email"></a> アラートの電子メール通知を設定します。  
...
-   ライセンス エラー:Active Directory Domain Services。  
...
###  <a name="BKMK_SMTP"></a> Windows Server Essentials での電子メールでアラート通知を送信するようにサーバーで SMTP を構成します。  
 このセクションでは、アラートの電子メール通知を送信するようにサーバーを設定する方法について説明します。  
  
> [!NOTE]
>  状態レポート アドインから Windows Server essentials をダウンロードすることができます、 [Microsoft ダウンロード センター](https://go.microsoft.com/fwlink/p/?LinkId=266342)します。  
...
4.  **[SMTP 設定]** ウィンドウで、次のように操作します。  
    ...
    1.  **[送信元電子メール アドレス]** に、電子メール アラートの送信元として使用する電子メール アドレスを入力します。 この電子メール アドレスがアラート通知に送信者のアドレスとして表示されます。  
    ...
    |-----------------|------------------|-----------------------------|-----------------|------------------------------|  
    |smtp.gmail.com|〇|〇|587|認証に必要なドメイン名とパスワードを完全電子メール アドレスに指定します。|  
    |smtp.live.com|〇|〇|587|認証に必要なドメイン名とパスワードを完全電子メール アドレスに指定します。|  
    |smtp.comcast.net|〇|X|587|認証に必要なドメイン名とパスワードを完全電子メール アドレスに指定します。|  
    |smtp.mail.yahoo.com|X|〇|25|ユーザー名にドメイン名のない電子メール アドレスのみを指定します。|  
...
### <a name="configuring-smtp-on-your-server-to-send-health-reports-in-windows-server-essentials"></a>Windows Server Essentials での正常性レポートを送信するようにサーバーで SMTP を構成します。  
 このセクションでは、電子メールで状態レポートを受信するようにサーバーの SMTP を設定する方法について説明します。  
  
> [!NOTE]
>  既定では、正常性レポートを追加では Windows Server Essentials または Windows Server 2012 R2、Windows Server Essentials エクスペリエンス役割をインストールすると統合の正常性レポートが表示されます、**正常性レポート** タブダッシュ ボードの**ホーム**ページ。  
...
4.  **[スケジュールと電子メール]** タブの **[電子メール]** セクションで、次のように操作します。  
    ...
    1.  **[有効]** をクリックし、状態レポートの送信元にする電子メール アドレスを入力します。 この電子メール アドレスが、電子メールで送信される状態レポートに送信者のアドレスとして表示されます。  
...
|ハード ドライブの空き領域が少なくなっています。|利用できる領域がない場合、追加情報を保存できなくなります。 コンピューターの空き領域を増やすには、次の選択肢があります。<br /><br /> -新しいハード ドライブを追加します。<br /><br /> -実行**ディスク クリーンアップ**古いおよび一時ファイルを削除します。<br /><br /> -別のコンピューターで共有フォルダーにファイルを移動します。<br /><br /> -CD、DVD、または外付けのハード ドライブなどのリムーバブル メディア上のファイルをアーカイブします。|  

...
>  Windows Server Essentials と Windows Server Essentials エクスペリエンス役割がインストールされた Windows Server 2012 R2 で、サーバーとネットワーク内のクライアント コンピューターの正常性アラートはアラート ビューアーで表示されなくなりますが、の代わりに表示できます。**正常性レポート**のタブ、**ホーム**ページ。  
 ...
 Windows Server Essentials サーバーに接続されていて、マルウェア対策クライアントの古いウイルス定義が不足している管理者が問題に関連する、重要な更新プログラムを含む、システムの正常性アラートすべてのコンピューターを積極的に監視します。コンピューター、およびその他の重要な問題に対処が必要です。 これらの問題はアラート ビューアーは、サーバーのダッシュ ボードまたはクライアント コンピューター s in Windows Server Essentials、またはスタート パッドから起動できるアラートとして表示されます、**正常性レポート**Windows Server Essentials でのタブ。 既定では、アラートは 30 分おきに更新されます。ただし、ネットワークのアラートはいつでも確認できます。アラート ビューアーまたは **[状態レポート]** タブの **[更新]** をクリックします。  
...
>   
>  **Windows Server Essentials:** 既定では、正常性レポートを追加では Windows Server Essentials または Windows Server 2012 R2、Windows Server Essentials エクスペリエンス役割をインストールすると統合の正常性レポートが表示されます、**正常性レポート** タブs ダッシュ ボードの**ホーム**ページ。  
...
-   **記憶域** ドライブとその容量の一覧が **[詳細]** セクションに表示されます。  
 ...
 既定でレポートに含まれるデータ ポイントの一部に興味がない場合、レポートのコンテンツをカスタマイズできます。タスク ウィンドウの **[状態レポートの設定のカスタマイズ]** をクリックし、**[コンテンツ]** タブをクリックします。表示されないが、レポートに表示するコンテンツのチェック ボックスをオフにします。 たとえば、独自のサーバー バックアップ計画があり、don t がサーバーのバックアップに関する警告を表示する場合から除外できますサーバー バックアップ、レポートをオフにして、**サーバー バックアップ**チェック ボックスをオンします。  
...
4.  **[SMTP 設定]** ウィンドウで、次のように操作します。  
    ...
    |smtp.comcast.net|〇|いいえ|587|認証に必要なドメイン名とパスワードを完全電子メール アドレスに指定します。|  
...
> [!NOTE]
>  既定では、正常性レポートを追加では Windows Server Essentials または Windows Server 2012 R2、Windows Server Essentials エクスペリエンス役割をインストールすると統合の正常性レポートが表示されます、**正常性レポート** タブs ダッシュ ボードの**ホーム**ページ。  
...
4.  **[スケジュールと電子メール]** タブの **[電子メール]** セクションで、次のように操作します。  
    ...
    1.  **[有効]** をクリックし、状態レポートの送信元にする電子メール アドレスを入力します。 電子メールで通知する正常性レポートで、送信者のアドレスとして、この電子メール アドレスが表示されます。  

...
2.  イントラネット経由で Windows Server Essentials サーバーに接続するためにサーバー名を使用できるようにし、デバイスに Windows 8 または Windows RT オペレーティングシステムがある場合は、次の手順を実行して、既定の CAROOT .cer 証明書をコンピューターにインストールします。作業. Windows 8.1 および Windows RT 8.1 では、証明書の手動インストールは不要です。  
